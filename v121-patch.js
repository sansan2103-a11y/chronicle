/* v121: bare-name vocative + inner-thought subject detection */
(function v121(){
  'use strict';
  var TAG = '[v121]';
  if (window.__v121Active) return;
  window.__v121Active = true;
  ...
  /* ===== ENHANCED Vocative: bare name OK ===== */
  /* "アリア、" "アリア…" "アリア・" "アリア〜" "アリアちゃん" "アリア様" "アリア!" "アリア?" all = addressing アリア */
  function findVocativeTargets(dlgText, allNames){
    var targets = [];
    allNames.forEach(function(name){
      if (!name) return;
      var esc = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      /* With or without honorific, but with terminal mark or address mark */
      var rxHonor = new RegExp(esc + '(?:さん|さま|様|くん|ちゃん|殿|氏)');
      var rxBare = new RegExp(esc + '(?:、|，|！|？|!|\\?|…|・|・・|〜|ー|っ|や|よ|ね|へ|に|を)');
      /* Standalone at start: "アリア、" "アリア…" */
  ...
  function attributeDialogue(narr, pos, dlgText, defSpk, heroName, npcs){
    ...
    /* 1. Explicit name prefix in narrative: "ミリアは「..」" */
    ...
    /* 1b. Postfix attribution: 「..」と<name>が言った/呟く/etc */
    var after = narr.slice(pos + dlgText.length + 2, pos + dlgText.length + 80);
    var postMatch = after.match(/^\s*[とは]?\s*([一-鿿ぁ-ゖァ-ヺ々ー・]{2,12})(?:は|が)?\s*[、。]?\s*(?:嗚咽|懇願|呟|言|答|叫|問|呼|尋|応|返|笑|囁|吐|怒鳴|頷|呻|喘)/);
    if (postMatch){
      var info2 = findCast(postMatch[1]);
      if (info2 && vocativeTargets.indexOf(info2.name) < 0) return info2;
    }

    /* 2. First-person pronoun → hero (unless hero is vocative target) */
    ...
    /* 3. Vocative implies speaker is NOT the addressed party */
    if (vocativeTargets.length > 0){
      /* If hero is the vocative target, speaker is NPC */
      if (vocativeTargets.indexOf(heroName) >= 0){
        for (var i = 0; i < npcs.length; i++){
          var n = npcs[i];
    ...
    /* 4. Subject of nearest preceding sentence */
    var window2 = narr.slice(Math.max(0, pos - 200), pos);
    /* Find subject: "<name>は" "<name>が" pattern */
    var subjMatches = [];
    ...
    if (subjMatches.length > 0){
      /* Pick the LAST subject (closest to dialogue) */
  ...
  /* ===== INNER THOUGHT: subject from surrounding narrative ===== */
  function attributeInnerThought(narr, pos, innerText, heroName, npcs){
    var allNames = [heroName].concat(npcs.map(function(n){ return n && n.name; })).filter(Boolean);

    /* 1. First-person pronoun → hero */
    if (FP_RX.test(innerText)){
      var hero = findCast(heroName);
      if (hero) return hero;
    }

    /* 2. Subject of prceding sentence */
    ...
    /* 3. Default: hero */
    var hero2 = findCast(heroName);
    return hero2;
  }

  /* ===== Re-render dialogue stream with v121 attribution ===== */
  function rerenderStreamV121(){
    ...
    turns.forEach(function(t){
      ...
      /* Collect both 「..」 and 《..》 with their positions */
      var items = [];
      var rxDlg = /「([^「」\n]{1,300})」/g;
      var rxInn = /《([^《》\n]{1,300})》/g;
      var m;
  ...
  console.log(TAG, 'v121 active: bare-vocative + inner-subject');
})();
