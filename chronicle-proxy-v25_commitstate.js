// ============================================================
// Chronicle APIプロキシ (Cloudflare Worker) — v25(save系) / v28(image系)
// ------------------------------------------------------------
...
//     (4) 新API op:'commitstate' … { rev, packageHash, lastCommitOpId, hashAlg, serverTs, tombstone? }
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 28, freeFallback: true, genBudget: gb23, inspect: true, inspectSpec: 'v20.5', avatarGuard: !!(cfg23 && cfg23.fix476Guard), avatarGuardSpec: 'v27.0', imgKeyMode: ((cfg23 && (cfg23.imgKeyMode === 'reserve' || cfg23.imgKeyMode === 'auto')) ? cfg23.imgKeyMode : 'primary'), paidKeySet: !!String(env.POLLINATIONS_KEY_PAID||'').trim(), lora420: true, debug420: true, style420: true, strict: String(env.ALLOW_IMAGE_FALLBACK||'') !== '1', d1: !!env.DB, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true,
        ...
        tombstoneGuard: true, workerBuild: 'v25',
        capabilities: { tombstoneGuard: 1, packageHash: HASH_ALG_V25, commitOpId: 1, commitState: 1 } }, 200, request);
    }
...
async function d1Migrate(env) {
  try {
    ...
    try { await env.DB.exec("ALTER TABLE saves ADD COLUMN package_hash TEXT"); } catch (e) {}
    try { await env.DB.exec("ALTER TABLE saves ADD COLUMN last_commit_op_id TEXT"); } catch (e) {}
    try { await env.DB.exec("ALTER TABLE saves ADD COLUMN hash_alg TEXT"); } catch (e) {}
...
async function d1PutImg(env, ns, k, data, baseRev) {
  ...
  const now = Date.now();
  // b(2026-07-21): baseRev given => conditional putimg (409 on rev mismatch). Absent => legacy unconditional upsert (fully backward compatible).
  if (baseRev != null) {
    const cur = await env.DB.prepare('SELECT rev, hash FROM images WHERE ns=?1 AND k=?2').bind(ns, k).first();
    if (cur && String(cur.hash) === hash) return { rev: +cur.rev || 0, hash, updatedAt: now, noop: true };
    if (!cur) { await env.DB.prepare('INSERT INTO images (ns, k, rev, hash, updatedAt, data) VALUES (?1,?2,1,?3,?4,?5)').bind(ns, k, hash, now, str).run(); return { rev: 1, hash, updatedAt: now }; }
    ...
    if (!d1Changed(r2)) { const e = new Error('image-conflict'); e.conflict = true; e.serverRev = +cur.rev || 0; throw e; }
...
async function handleSave(request, env, ctx) {
  ...
  try {
    ...
    if (op === 'commitstate') {
      ...
      const row = await env.DB.prepare('SELECT rev, blob, package_hash, last_commit_op_id, hash_alg, updatedAt, device, size FROM saves WHERE u=?1 AND kind=?2').bind(user, 'main').first();
      if (!row) return json({ ok: true, rev: 0, packageHash: null, lastCommitOpId: null, hashAlg: HASH_ALG_V25, serverTs: Date.now(), exists: false, requestId }, 200, request);
      ...
      if (!ph && row.blob) { ph = await sha256Utf8v1(String(row.blob)); alg = HASH_ALG_V25; computed = true; }
      const slotId = (body && body.slotId != null && body.slotId !== '') ? String(body.slotId).slice(0, 64) : null;
      const out = {
        ok: true, rev: +row.rev || 0, packageHash: ph, lastCommitOpId: row.last_commit_op_id ? String(row.last_commit_op_id) : null,
        hashAlg: alg || HASH_ALG_V25, hashComputedOnRead: computed, serverTs: Date.now(),
        updatedAt: +row.updatedAt || 0, size: +row.size || 0, exists: true, requestId
      };
        ...
        if (op === 'forceput') {
          ...
          const attempt = async (n) => {
            ...
            const res = await env.DB.batch([
              ...
              env.DB.prepare('UPDATE saves SET rev=rev+1, baseRev=?3, updatedAt=?4, device=?5, size=?6, blob=?7, package_hash=?8, last_commit_op_id=?9, hash_alg=?10 WHERE u=?1 AND kind=?2 RETURNING rev')
                .bind(user, 'main', newBase, now, dev, str.length, str, pkgHash25, commitOpId25, HASH_ALG_V25)
            ]);
            const upd = res && res[1];
            if (d1Changed(upd)) return d1FirstRev(upd, null);
            // main無し→新規作成(INSERT OR IGNORE)。他が先に作っていれば0行。
            const ins = await env.DB.prepare('INSERT OR IGNORE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob, package_hash, last_commit_op_id, hash_alg) VALUES (?1,?2,1,?3,?4,?5,?6,?7,?8,?9,?10) RETURNING rev')
              .bind(user, 'main', newBase, now, dev, str.length, str, pkgHash25, commitOpId25, HASH_ALG_V25).run();
          ...
          return okJson({ ok: true, rev: frev, size: str.length, ns, packageHash: pkgHash25, lastCommitOpId: commitOpId25, hashAlg: HASH_ALG_V25, requestId });
        }

        // ★B-1: op:put の原子的コミット。条件付きUPDATE(cur有)/INSERT OR IGNORE(新規)。
        const newBase = hasBase ? baseRev : curRev;
        if (cur) {
          const upd = await env.DB.prepare('UPDATE saves SET rev=rev+1, baseRev=?3, updatedAt=?4, device=?5, size=?6, blob=?7, package_hash=?9, last_commit_op_id=?10, hash_alg=?11 WHERE u=?1 AND kind=?2 AND rev=?8 RETURNING rev')
            .bind(user, 'main', newBase, now, dev, str.length, str, curRev, pkgHash25, commitOpId25, HASH_ALG_V25).run();
          if (!d1Changed(upd)) {
          ...
          return okJson({ ok: true, rev: d1FirstRev(upd, curRev + 1), size: str.length, ns, packageHash: pkgHash25, lastCommitOpId: commitOpId25, hashAlg: HASH_ALG_V25, requestId });
        } else {
          const ins = await env.DB.prepare('INSERT OR IGNORE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob, package_hash, last_commit_op_id, hash_alg) VALUES (?1,?2,1,?3,?4,?5,?6,?7,?8,?9,?10) RETURNING rev')
            .bind(user, 'main', newBase, now, dev, str.length, str, pkgHash25, commitOpId25, HASH_ALG_V25).run();
          if (!d1Changed(ins)) {
          ...
          return okJson({ ok: true, rev: 1, size: str.length, ns, packageHash: pkgHash25, lastCommitOpId: commitOpId25, hashAlg: HASH_ALG_V25, requestId });
        }
      }
