...
(function(){
  ...
  function getS(){
    ...
    if (typeof g === 'function'){
      try { var a = g('fix409'); if (a) return a; } catch(e){ note539('fix409', 'getter-threw', e); }
    } else { note539('fix409', 'getter-missing'); }
    ...
    try { if (window.S){ note539('fix409', 'rescued-by-window'); return window.S; } } catch(e){}
    try { if (typeof S !== 'undefined' && S){ note539('fix409', 'rescued-by-lexical'); return S; } } catch(e){}
    try { var u = (0,eval)('typeof S!=="undefined"?S:null');
          if (u){ note539('fix409', 'rescued-by-eval'); return u; }
          note539('fix409', 'legacy-eval-null'); }
    catch(e){ note539('fix409', 'legacy-eval-threw', e); }
    return null;
  }
  ...
  function repair(_auto){
    ...
    for (var di = 0; di < S.turns.length; di++){
      if (_auto && !_f732New(S.turns[di])) continue;    /* ★★fix732: 自動経路は session-new turn のみ */
    ...
    for (var ti = 0; ti < S.turns.length; ti++){
      if (_auto && !_f732New(S.turns[ti])) continue;    /* ★★fix732: 同上（本適用ループ） */
  ...
  function tick(){
    try {
      ...
      lastSig = sig;
      repair(true);   /* ★fix732: 自動経路 */ // fix409b(c): inFlight中なら repair 内で lastSig=null に戻り次tickで再評価
    } catch(e){}
  }
