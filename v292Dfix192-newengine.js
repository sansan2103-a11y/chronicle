...
(function(){
  ...
  function buildSys(mode, text){
    ...
    blocks = blocks.concat([
      ...
      (lenKey()==='long'
        ? '本文は地の文と「」セリフで、12〜20文。一つの場面を急がず、情景→身体感覚→感情→行動の順に層を重ねて厚く描く。段落を分けてよい。ただし同じ内容の言い換え・繰り返しで水増ししない。下の見本は構造の参考であって長さの上限ではない。見本より大幅に長く、必ず12文以上書く。'
        : lenKey()==='short'
    ...
    blocks = blocks.concat([
      ...
      '',
      miniEx() + '\n\n' + (EXAMPLES[toneKey()] || EXAMPLES['shizuka'])   // v292Dfix208: 🎭トーンで見本切替 / fix223d改: ミニ見本は短いので先に置き、長いトーン見本を最後に(=直近アンカー)。fix212の見本アンカリング実証より
      + (lenKey()==='long' ? '\n\n（最重要リマインド: 本文は必ず12文以上。見本は構造の参考で、長さの上限ではない）' : '')
    ]);
    return blocks.join('\n');
  }
