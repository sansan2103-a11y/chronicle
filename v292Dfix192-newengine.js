...
(function(){
  ...
  function buildSys(mode, text){
    ...
    blocks = blocks.concat([
      ...
      '【セリフと物音の区別】声に出した言葉だけが<say>。物音・気配・息遣いは地の文で描き、セリフにしない。',
      '',
      '【守ること】',
      ...
      '',
      miniEx() + '\n\n' + (EXAMPLES[toneKey()] || EXAMPLES['shizuka'])   // v292Dfix208: 🎭トーンで見本切替 / fix223d改: ミニ見本先・トーン見本後 / fix224: 強制リマインドは実測無効のため撤去(分量はモデル得意域に合わせる方針)
    ]);
    return blocks.join('\n');
  }
