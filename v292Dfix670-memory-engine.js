/* =====================================================================
 * v292Dfix670-memory-engine.js — Memory Engine v0.2 / Phase 1 Shadow
 * ---------------------------------------------------------------------
 * ■何をするか
...
(function () {
  ...
  var TAG = '[v292Dfix670]';
  var BUILD = '20260803-fix670';
  var SCHEMA_VERSION = 3;   /* ★chain に entityContextHash が増えた */
  var EXTRACTOR_VERSION = 'me-0.2.2';      /* ★抽出規則が変わったので上げる（鍵の作り方は v0.2.0 と同じ） */
  var DB_NAME = 'chr6mem';                 /* ★chr6av(アバター)には相乗りしない */
  var DB_VER = 3;                          /* ★件数を count() で取るための index を追加 */
  ...
  var TYPES = [
    ...
    { type: 'terrain_changed',    re: /(崩落|崩れ落ち|塞がれ|埋まっ|開通|焼け落ち|橋が落ち)/, cat: 'place' },
    ...
    { type: 'seal_state_changed', re: /(封印(が|を)(破|解|砕)|結界(が|を)(破|解))/,  cat: 'place' },
  ...
  function findRefs(span, known) {
    ...
    for (i = 0; i < names.length; i++) {
      ...
      if (p >= 0) hits.push({ name: names[i], pos: p });
    }
    ...
    for (i = 0; i < hits.length; i++) {
      ...
      out.push({ entityId: e.entityId, entityType: e.entityType, status: e.status,
                 ambiguous: !!e.ambiguous, raw: hits[i].name, pos: hits[i].pos });
    }
    return out;
  }
  /* 素の { 名前: 'char' } 形式でも動くようにする（旧試験・呼び出しの後方互換） */
  function asKnown(known) {
    if (known && known.byName && known.order) return known;
    var src = known || {}, map = {};
    ...
    return { byName: map, order: Object.keys(map).sort() };
  }
  ...
  function refView(r) {
    return r ? { entityId: r.entityId, raw: r.raw, entityType: r.entityType,
                 status: r.status, ambiguous: !!r.ambiguous } : null;
  }
      ...
      for (j = 0; j < spans.length; j++) {
        ...
        for (t = 0; t < TYPES.length; t++) {
          ...
          if (type === 'time_loop_detected') {
            res.unhandled.push({ type: type, sourceTurnIndex: idx, evidence: span.slice(0, EVIDENCE_MAX),
          ...
          var rec = {
            type: type, category: TYPES[t].cat, sourceTurnIndex: idx,
  ...
  function touchEntity(map, slotId, ref, idx) {
    ...
    if (!e) {
      map[id] = { slotId: slotId, entityId: id, name: ref.raw, entityType: ref.entityType,
  ...
  window.__v292Dfix670 = {
    ...
    rebuild: function () {                                        /* ★手動のみ。消してから単一versionで作り直す */
      var sl = resolveSlot(); if (!sl.ok) return Promise.resolve(sl.code);
      return window.__v292Dfix670.clear(sl.slotId).then(function () {
  ...
  try {
    console.log(TAG, st.armed
      ? ('shadow armed (build ' + BUILD + ', extractor ' + EXTRACTOR_VERSION + ')')
