...
(function(){
  ...
  /* ■fix784(2026-09-01) MULTI_TAB Tier3: backup provenance(控えの出所)
     真因: 控えの**元キー**が共有ポインタ chr6_active_slot(= __chr6Key()) 由来だったため、
       別タブが物語を開いた瞬間に **chr6_bk_fix445_<ts> の控えとして別 story の本体が保存される**。
  ...
  function activeSlotKey(){
    if (!f784Off()){
      try { var dk = G.__chronicleDocumentStoryKey; if (typeof dk === 'string' && dk) return dk; } catch(e){}
  ...
  function backupOnce(reason){
    ...
    try {
      ...
      if (!ak) return false;                           /* ■fix784: authority 無し = 控えない → 既存 fail-closed */
      var blob = ls(ak) || '';
