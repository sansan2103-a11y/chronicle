...
(function v292Dfix648(){
  ...
  function wrapParse(){
    var P, R = null;
    if (!PAGE_CANARY){
      /* ===== OFF: production 経路（production v292Dfix648 :128-130 statement 逐語） ===== */
      P = getPlanner();
      if (!P || typeof P.parsePlan !== 'function') return false;
      if (P.parsePlan.__f648){ wired.parsePlan = true; return true; }
    } else {
      /* ===== ON: frozen Phase A core 経路（candidate/q119 :132-137 逐語） ===== */
      R = reg648();
      if (R.f648parse) return true;                 /* ★Q119-A INSTALL_ONCE_PER_PAGE */
      P = getPlanner();
      if (!P || typeof P.parsePlan !== 'function') return false;
      if (P.parsePlan.__f648){ wired.parsePlan = true; R.f648parse = true; return true; }
    }
    ...
    if (PAGE_CANARY) R.f648parse = true;            /* ★registry が真実（ON のみ） */
