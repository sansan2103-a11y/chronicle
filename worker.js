  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 28, freeFallback: true, genBudget: gb23, inspect: true, inspectSpec: 'v20.5', avatarGuard: !!(cfg23 && cfg23.fix476Guard), avatarGuardSpec: 'v27.0', imgKeyMode: ((cfg23 && (cfg23.imgKeyMode === 'reserve' || cfg23.imgKeyMode === 'auto')) ? cfg23.imgKeyMode : 'primary'), paidKeySet: !!String(env.POLLINATIONS_KEY_PAID||'').trim(), lora420: true, debug420: true, style420: true, strict: String(env.ALLOW_IMAGE_FALLBACK||'') !== '1', d1: !!env.DB, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true,
        ...
        tombstoneGuard: true, workerBuild: 'v41.0-storypass', storyShadow: 1, storyAuthority: 1, shadowDelete: 1, canonicalWrite: 1, cfgAllowlist: 1, canonicalDelete: 1, cfgScrub: 1, storyTitleWrite: 1, canonicalSchemaMax: 2,
        ...
        capabilities: { tombstoneGuard: 1, packageHash: HASH_ALG_V25, commitOpId: 1, commitState: 2, longLivedSession: 1 } }, 200, request);
    }
...
/* ★★fix836 GOOGLE_AUTH_LONG_LIVED_SESSION_V1（設計 v1・②GPT 裁定 2026-09-08）
   背景（実測）: ID トークンの寿命は 1 時間で、refresh token を持つ経路がこの製品に存在しない。
     そのため 1 時間ごとに cloud write が止まり、client 側では原理的に直せない（fix835 は表出のみ修正）。
   契約:
     ・Google 検証に成功した「事実」を、不透明な session id として **既存 LEDGER KV のキー接頭辞** sess: に置く。
     ・TTL は 7 日（GPT 裁定）。KV の expirationTtl は本 Worker で既に使われている既存パターン。
     ・**認可（active / 月次上限）はセッション経路でも毎回引き直す**。判断結果をキャッシュしない。
     ・codeKey は 'allow:'+email のままで、resolveUser / link / autoLink の意味論は 1 行も変えない。
     ・**session 経路では gate.email を返さない** → autoLink は発火しない（lane 張り替えを起こさない）。
     ・**binding を 1 つも増やさない**（NO_BINDING_CHANGE。増やすと Cloudflare 側で rollback が拒否される）。
   互換:
     ・checkAuth の順序は session → google → pass。session を送らない旧 client は完全に従来どおり。
     ・session が無効・期限切れなら 401 にせず下の経路へ落とす（Google / 合言葉で従来どおり通る）。 */
const CHR_SESS_PREFIX  = 'sess:';
const CHR_SESS_TTL_SEC = 60 * 60 * 24 * 7;   /* 7 日 */

function chrNewSessionId() {
...
async function chrSessionMint(env, email, codeKey) {
  const sid = chrNewSessionId();
  await env.LEDGER.put(CHR_SESS_PREFIX + sid,
    JSON.stringify({ codeKey: String(codeKey || ''), email: String(email || '').toLowerCase(), createdAt: Date.now() }),
    { expirationTtl: CHR_SESS_TTL_SEC });
  return sid;
}
async function chrSessionResolve(env, sid) {
  if (!sid || !env.LEDGER) return null;
  let raw = null;
  try { raw = await env.LEDGER.get(CHR_SESS_PREFIX + sid); } catch (e) { return null; }
  if (!raw) return null;
  try { const o = JSON.parse(raw); return (o && o.email) ? o : null; } catch (e) { return null; }
}

// ---------- 認証: Google(x-google-id) を優先、無ければ合言葉(x-chronicle-pass) ----------
async function checkAuth(request, env, opts) {
  /* ★★v40.3 H-1 BUDGET_GATE_BILLABLE_ONLY (②C1 裁定 2026-09-14)
     旧: 月次予算超過(402)が checkAuth で効くため、**課金が発生しない /save**
     （putstory / getstory / putcanonical / meta / session など）まで止まり、
     利用者には「クラウド保存ができなくなった」という形で現れていた。
     新: opts.enforceBudget === false のときは 402 で落とさず、gate に
     budgetExceeded:true を載せて返す。既定は true なので **他の経路の挙動は不変**。
     課金経路（/ 生成・/inspect・/image）は従来どおり 402 で止まる。 */
  const enforceBudget = !(opts && opts.enforceBudget === false);
  /* ★fix836: session → google → pass。session は「Google 検証済みという事実」の再利用にすぎず、
     認可は毎回引き直す。email を返さないので autoLink は発火しない。 */
  const sid836 = request.headers.get('x-chronicle-session') || '';
  if (sid836 && env.LEDGER) {
    const sess836 = await chrSessionResolve(env, sid836);
    if (sess836) {
      const email836 = String(sess836.email || '').toLowerCase();
      const config836 = await getJSON(env, 'config', {});
      const rec836 = await getJSON(env, 'allow:' + email836, null);
      if (!rec836 || !rec836.active) {
        return { ok: false, status: 403, error: 'このGoogleアカウント(' + email836 + ')はまだ許可されていません。管理者に連絡してください' };
      }
      const m836 = month();
      const used836 = (rec836.month === m836) ? (+rec836.usedUsd || 0) : 0;
      const over836 = (+rec836.limitUsd > 0 && used836 >= +rec836.limitUsd);
      if (over836 && enforceBudget) {
        return { ok: false, status: 402, error: '今月の利用上限に達しました(管理者に連絡してください)' };
      }
      return { ok: true, codeKey: 'allow:' + email836, config: config836, session: true, budgetExceeded: over836 };
    }
    /* 無効/期限切れ: ここで落とさず、Google / 合言葉の従来経路へフォールバックする（後方互換） */
  }
  ...
  if (gid) {
    ...
    const over = (+rec.limitUsd > 0 && used >= +rec.limitUsd);
    if (over && enforceBudget) {
      return { ok: false, status: 402, error: '今月の利用上限に達しました(管理者に連絡してください)' };
    }
    return { ok: true, codeKey: 'allow:' + email, config, email, budgetExceeded: over };
  }
  const pass = request.headers.get('x-chronicle-pass') || '';
  return checkPass(pass, env, enforceBudget);
}
...
async function verifyGoogleIdToken(idToken, clientId) {
  ...
  /* ★★v40.3 H-2 GOOGLE_CLIENT_ID_FAIL_CLOSED (②C1 裁定 2026-09-14)
     旧: clientId が falsy だと audience 検査ごと skip され、**任意の Google プロジェクトが
     発行した id token でも通ってしまう**（認証は成功するので誰も気づかない）。
     新: Google 経路では clientId 未設定を **fail-closed** にする。合言葉経路は無影響。 */
  if (!clientId) throw new Error('GOOGLE_CLIENT_ID is not configured on this Worker');
...
async function checkPass(pass, env, enforceBudget) {
  /* ★v40.3 H-1: 呼び出し元が省略したときは従来どおり enforce（安全側の既定）。 */
  if (enforceBudget === undefined) enforceBudget = true;
  ...
  const m = month();
  const used = (rec.month === m) ? (+rec.usedUsd || 0) : 0;
  const overP = (+rec.limitUsd > 0 && used >= +rec.limitUsd);
  if (overP && enforceBudget) {
    return { ok: false, status: 402, error: '今月の利用上限に達しました(管理者に連絡してください)' };
  }
  return { ok: true, codeKey: 'code:' + pass, config, budgetExceeded: overP };
}
...
async function record(env, codeKey, text) {
  try {
    ...
    /* ★★v40.3 H-4 NO_AUTHORIZATION_ROW_RESURRECTION (②C1 裁定 2026-09-14)
       旧: 行が無いと {active:true, limitUsd:0} を既定にして put していたため、
       管理者が allow/code を削除・失効させた直後に in-flight の記帳が走ると
       **無制限で有効な行として蘇る**（revoke が効いたように見えて効かない）。
       新: 既存行が無ければ記帳しない。会計は落ちるが認可が壊れるより安全。 */
    const rec = await getJSON(env, codeKey, null);
...
async function recordImgUser(env, codeKey, unit) {
  try {
    ...
    /* ★★v40.3 H-4: 画像側も同じ。行が無ければ記帳しない（蘇生させない）。 */
    const rec = await getJSON(env, codeKey, null);
    if (!rec || typeof rec !== 'object') return;
    const m = month();
    if (rec.month !== m) { rec.month = m; rec.usedUsd = 0; rec.reqs = 0; rec.tokens = 0; }
...
function cors(request) {
  ...
  return {
    ...
    /* ★★fix839 CORS_ALLOW_SESSION_HEADER: fix836 で x-chronicle-session を導入したとき、
       この許可リストの更新を落とした。client（github.io）→ Worker（workers.dev）は
       cross-origin なので、許可リストに無いカスタムヘッダを付けると
       preflight でブラウザが本リクエストを拒否する（= cloud write が全部止まる）。 */
    'Access-Control-Allow-Headers': 'Content-Type, x-chronicle-pass, x-admin-token, x-google-id, x-chronicle-session',
...
/* ★★v40.3 H-3 STORY_BLOB_SIZE_GUARD (②C1 裁定 2026-09-14)
   story_shadow.blob には chunking が無く、size guard も無かった（legacy put は 4MB、
   putimg は 2MB の guard がある）。巨大 blob は .run() 内で throw し、handleSave の
   catch で errorCode:'exception' / **retryable:true** の 500 になるため、client が
   永久に retry する。閾値は推測せず、**同じ worker が inline 安全域として既に採用している
   CHUNK_THRESHOLD_V27 と同じ 1,000,000（JS 文字長）**を使う。
   実測(2026-09-14, canonical 31 本): schema1 canonicalString max 84,701 B /
   schema2 JSON max 87,257 B(UTF-8) / 最大の local story(Owner) 198,333 文字。
   → 実測最大のおよそ 5 倍の余裕があり、既存 story も Owner story もこの guard に触れない。 */
const CHR_STORY_BLOB_MAX_V403 = 1000000;
...
async function handleSave(request, env, ctx) {
  ...
  try {
    ...
    /* ★★v40.3 H-1: /save は課金経路ではない（record/recordImgUser を 1 箇所も呼ばない）。
       予算超過で putstory / getstory / putcanonical / meta / session まで止めない。
       予算が尽きているかどうかは gate.budgetExceeded で観測できる。 */
    const gate = await checkAuth(request, env, { enforceBudget: false });
    ...
    /* ★fix836: セッション発行。**Google 検証で入ってきたときだけ**発行する
       （gate.email は Google 経路でのみ立つ。session 経路・合言葉経路では立たない）。
       書くのは KV の sess:<sid> 1 本だけ。D1 には触れない。 */
    if (op === 'session') {
      if (!gate.email) return json({ ok: false, error: 'session requires Google login', errorCode: 'unsupported', retryable: false, requestId }, 400, request);
      const sid = await chrSessionMint(env, gate.email, gate.codeKey);
      return json({ ok: true, session: sid, ttlSec: CHR_SESS_TTL_SEC, requestId }, 200, request);
    }
    /* ★fix836: セッション失効（ログアウト）。KV の 1 キーを消すだけ。 */
    if (op === 'sessionend') {
      const sidEnd = String((body && body.session) || request.headers.get('x-chronicle-session') || '').slice(0, 128);
      if (sidEnd) { try { await env.LEDGER.delete(CHR_SESS_PREFIX + sidEnd); } catch (e) {} }
      return json({ ok: true, requestId }, 200, request);
    }
    ...
    if (op === 'get' || op === 'getfork') {
      /* ★★STORY_PASSCODE 裁定 案(b): account 内に locked story が 1 件でもあれば、
         全 story を内包する legacy package を **読ませない**（T4 の根治）。fail-closed。 */
      if (d1 && await spAccountHasLock(env, user)) {
        return json({ ok: false, error: 'この物語一式にはパスコードで保護された物語が含まれているため、旧経路では取得できません',
          errorCode: 'LEGACY_PATH_BLOCKED_BY_STORY_LOCK', legacyBlocked: true, retryable: false, requestId }, 409, request);
      }
      if (d1) {
    ...
    /* ★★STORY_PASSCODE: passcode lifecycle ops（story body には触れない） */
    if (SP_PASS_OPS[op]) {
      if (!d1) return json({ ok: false, storyPass: true, error: 'd1 required', errorCode: 'no-d1', retryable: false, requestId }, 503, request);
      const pr = await spHandlePassOp(env, request, user, op, body, requestId);
      return json(pr.obj, pr.status, request);
    }
    if (op === 'putstory' || op === 'getstory' || op === 'listshadow'
      ...
      const rr = await handleStoryShadow(env, user, op, body, mid, requestId, request);
    ...
    if (op === 'put' || op === 'forceput') {
      /* ★★STORY_PASSCODE 裁定 案(b): locked story を含む account では legacy package の
         **書き戻しも止める**（T5 の根治）。fail-closed。 */
      if (d1 && await spAccountHasLock(env, user)) {
        return json({ ok: false, error: 'パスコードで保護された物語があるため、旧経路では保存できません',
          errorCode: 'LEGACY_PATH_BLOCKED_BY_STORY_LOCK', legacyBlocked: true, retryable: false, requestId }, 409, request);
      }
...
// =====================================================================
// ★★ STORY_PASSCODE v1 (②C1 裁定 2026-09-16 / STORY_PASSCODE_ACCESS_CONTROL_LANE)
// ---------------------------------------------------------------------
// 目的: story 単位の passcode を **server 側で強制**する。localStorage PIN ではない。
//
// 権威モデル（裁定 Security correction 1）:
//   ・KV `storysess:` は **session storage にすぎず最終 authority ではない**。
//   ・protected request のたびに (1) raw token 受領 (2) SHA-256(token) を KV lookup key に使用
//     (3) KV record の u / sid / exp を検証 (4) exp を Worker 自身でも検証
//     (5) ★D1 story_shadow の現在の lockVersion と一致確認 — を必ず行う。
//   ・★**D1 の現在の lock 状態が最終 authority**。passcode 変更後、古い KV session が
//     他 PoP に一時的に残っても lockVersion mismatch で即拒否される。
//   ・lockVersion は新 column を作らず、現在の pass_hash|pass_salt|pass_alg|pass_updated_at
//     から安定導出する（SHA-256 の先頭 32 hex）。
//
// brute force（裁定 Security correction 2）:
//   ・Workers の rate-limit binding は authority にしない。★**D1 の
//     pass_fail_count / pass_fail_window_at / pass_lock_until が authoritative**。
//   ・lockout 中は ★**PBKDF2 を回す前に拒否**する（CPU を焼かせない）。
//   ・正解 unlock で失敗状態を reset。
//
// 漏洩禁止: pass_hash / pass_salt / pass_alg / session token / token hash を
//   story JSON・legacy package・HOME 一覧・error body・console・log に **出さない**。
//
// iteration: ★benchmark 前は SP_ITER = 0 = **機能無効(fail-closed)**。
//   この build では passcode を 1 件も作れない（storypassset が 409 で拒否する）。
//   benchmark 後に SP_ITER を固定し pass_alg = 'pbkdf2-sha256:i=<N>:v1' で freeze する。
// =====================================================================
const SP_ALG_VER  = 'v1';
const SP_ITER     = 0;            // ★benchmark 未実施 = 0 = disabled（final build で確定値に置換）
const SP_ITER_MAX = 600000;       // hard cap（benchmark 候補もこれを超えない）
const SP_SESSION_TTL_SEC = 8 * 3600;
const SP_MAX_FAILS       = 5;
const SP_FAIL_WINDOW_MS  = 15 * 60 * 1000;
const SP_LOCKOUT_MS      = 15 * 60 * 1000;
const SP_PASS_MIN = 4, SP_PASS_MAX = 128;
const SP_PROTECTED_OPS = { getstory:1, putstory:1, putcanonical:1, promotestory:1, promotedelete:1,
                           deleteshadow:1, deletecanonical:1, scrubstorycfg:1, setstorytitle:1 };
const SP_PASS_OPS = { storyunlock:1, storypassset:1, storypassclear:1, storypassstatus:1, storypassbench:1 };
const SP_COLS = 'pass_hash, pass_salt, pass_alg, pass_updated_at, pass_fail_count, pass_fail_window_at, pass_lock_until';
...
async function spSha256Hex(str) {
  return spHex(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(String(str))));
}
function spRandomHex(nBytes) {
  const b = new Uint8Array(nBytes); crypto.getRandomValues(b); return spHex(b.buffer);
}
/* 定数時間比較（長さ差も早期 return しない） */
function spEq(a, b) {
...
function spAlgString(iter) { return 'pbkdf2-sha256:i=' + iter + ':' + SP_ALG_VER; }
function spParseAlg(alg) {
  const m = /^pbkdf2-sha256:i=(\d+):(v\d+)$/.exec(String(alg || ''));
  if (!m) return null;
  const iter = +m[1];
  if (!(iter > 0) || iter > SP_ITER_MAX) return null;
  return { iter, ver: m[2] };
}
async function spDerive(pass, saltHex, iter) {
  const key = await crypto.subtle.importKey('raw', new TextEncoder().encode(String(pass)),
...
/* ★lockVersion: 新 column を作らず現在の lock metadata から安定導出する */
async function spLockVersion(row) {
  if (!row || row.pass_hash == null) return '';
  return (await spSha256Hex(
    String(row.pass_hash) + '|' + String(row.pass_salt) + '|' +
    String(row.pass_alg)  + '|' + String(row.pass_updated_at))).slice(0, 32);
}
function spIsLocked(row) { return !!(row && row.pass_hash != null && row.pass_hash !== ''); }

async function spLoadRow(env, user, sid) {
  return await env.DB.prepare(
    'SELECT rev, content_hash, deleted, authority, ' + SP_COLS +
    ' FROM story_shadow WHERE u=?1 AND story_id=?2').bind(user, sid).first();
}
/* account 内に locked story が 1 件でもあるか（legacy gate 用・本文は読まない） */
async function spAccountHasLock(env, user) {
  try {
    const r = await env.DB.prepare(
      'SELECT 1 AS x FROM story_shadow WHERE u=?1 AND pass_hash IS NOT NULL LIMIT 1').bind(user).first();
    return !!r;
  } catch (e) { return true; }   // ★判定不能なら fail-closed（legacy を塞ぐ側に倒す）
}

/* ---- session（KV は storage。authority は D1） ---- */
async function spSessionIssue(env, user, sid, lockVersion) {
  const token = spRandomHex(32);
  const k = 'storysess:' + (await spSha256Hex(token));
  const exp = Date.now() + SP_SESSION_TTL_SEC * 1000;
  await env.LEDGER.put(k, JSON.stringify({ u: user, sid, lockVersion, exp }),
    { expirationTtl: SP_SESSION_TTL_SEC });
  return { token, exp };
}
async function spSessionVerify(env, request, body, user, sid, row) {
  const raw = String((request && request.headers.get('x-chronicle-story-session')) ||
                     (body && body.storySession) || '').trim();
  if (!raw) return { ok: false, reason: 'no-session' };
  if (!env.LEDGER) return { ok: false, reason: 'no-kv' };
  let rec = null;
  try { rec = await getJSON(env, 'storysess:' + (await spSha256Hex(raw)), null); } catch (e) {}
  if (!rec) return { ok: false, reason: 'unknown-session' };
  if (String(rec.u) !== String(user)) return { ok: false, reason: 'user-mismatch' };
  if (String(rec.sid) !== String(sid)) return { ok: false, reason: 'sid-mismatch' };
  if (!(+rec.exp > Date.now())) return { ok: false, reason: 'expired' };
  const cur = await spLockVersion(row);
  if (!cur || !spEq(rec.lockVersion, cur)) return { ok: false, reason: 'lock-version-mismatch' };
  return { ok: true };
}

/* ---- brute force 状態（D1 が authoritative） ---- */
function spLockedOut(row, now) { return !!(row && +row.pass_lock_until > now); }
async function spNoteFail(env, user, sid, row, now) {
  const inWindow = (+row.pass_fail_window_at > 0) && (now - +row.pass_fail_window_at < SP_FAIL_WINDOW_MS);
  const count = (inWindow ? (+row.pass_fail_count || 0) : 0) + 1;
  const windowAt = inWindow ? +row.pass_fail_window_at : now;
  const lockUntil = (count >= SP_MAX_FAILS) ? (now + SP_LOCKOUT_MS) : null;
  await env.DB.prepare('UPDATE story_shadow SET pass_fail_count=?3, pass_fail_window_at=?4, pass_lock_until=?5 WHERE u=?1 AND story_id=?2')
    .bind(user, sid, count, windowAt, lockUntil).run();
  return { count, lockUntil };
}
async function spResetFail(env, user, sid) {
  await env.DB.prepare('UPDATE story_shadow SET pass_fail_count=0, pass_fail_window_at=NULL, pass_lock_until=NULL WHERE u=?1 AND story_id=?2')
    .bind(user, sid).run();
}
function spValidPass(p) {
  const s = String(p == null ? '' : p);
  return (s.length >= SP_PASS_MIN && s.length <= SP_PASS_MAX) ? s : null;
}

/* ---- passcode lifecycle ops ---------------------------------------- */
/*  storypassstatus : { id } → { locked, lockedOut, alg? } ※hash/salt は返さない
    storyunlock     : { id, pass } → { token, exp }（token は raw・**1 回だけ**返す）
    storypassset    : { id, pass } 初回設定 = account auth のみ・条件付き単一 UPDATE
                      { id, pass, current? } 変更 = ★valid story session ＋ lockVersion 一致 必須
    storypassclear  : { id } 解除 = ★valid story session ＋ lockVersion 一致 必須
    storypassbench  : ★一時診断。story body へのアクセス 0 / iteration に hard cap /
                      bounded execution。**final build では削除する。**                        */
async function spHandlePassOp(env, request, user, op, body, requestId) {
  const bad = (code, msg, status) => ({ status: status || 400, idemDone: false,
    obj: { ok: false, storyPass: true, error: msg, errorCode: code, retryable: false, requestId } });
  if (!env.DB)     return bad('no-d1', 'd1 required', 503);
  if (!env.LEDGER) return bad('no-kv', 'kv required', 503);

  if (op === 'storypassbench') {
    /* ★一時診断 op。story body には一切触れない。候補は hard cap と件数上限で bounded。 */
    const cand = Array.isArray(body && body.iters) ? body.iters.slice(0, 6) : [];
    const out = [];
    for (let i = 0; i < cand.length; i++) {
      const n = Math.floor(+cand[i] || 0);
      if (!(n > 0) || n > SP_ITER_MAX) { out.push({ iter: n, skipped: 'out-of-range' }); continue; }
      const salt = spRandomHex(16), t0 = Date.now();
      try { await spDerive('benchmark-not-a-real-passcode', salt, n); }
      catch (e) { out.push({ iter: n, error: 'derive-failed' }); continue; }
      out.push({ iter: n, ms: Date.now() - t0 });
    }
    return { status: 200, idemDone: false,
      obj: { ok: true, storyPass: true, bench: out, iterMax: SP_ITER_MAX, algVer: SP_ALG_VER, requestId } };
  }

  const sid = (body && body.id != null && body.id !== '') ? String(body.id).slice(0, 80) : '';
  if (!sid) return bad('bad-request', 'no id');
  const row = await spLoadRow(env, user, sid);
  if (!row) return bad('not-found', 'story not found', 404);
  const now = Date.now();

  if (op === 'storypassstatus') {
    return { status: 200, idemDone: false, obj: { ok: true, storyPass: true, id: sid,
      locked: spIsLocked(row), lockedOut: spLockedOut(row, now), requestId } };
  }

  if (op === 'storyunlock') {
    if (!spIsLocked(row)) return bad('not-locked', 'story is not locked', 409);
    /* ★lockout 中は PBKDF2 を回す前に拒否する */
    if (spLockedOut(row, now)) {
      return { status: 429, idemDone: false, obj: { ok: false, storyPass: true, id: sid,
        error: '試行回数が多すぎます。しばらく待ってからやり直してください',
        errorCode: 'STORY_PASSCODE_LOCKED_OUT', retryAfterMs: Math.max(0, +row.pass_lock_until - now),
        retryable: true, requestId } };
    }
    const pass = spValidPass(body && body.pass);
    if (!pass) return bad('STORY_PASSCODE_INVALID', 'passcode invalid', 403);
    const alg = spParseAlg(row.pass_alg);
    if (!alg) return bad('STORY_PASSCODE_ALG_UNSUPPORTED', 'stored algorithm is not supported', 500);
    let got = '';
    try { got = await spDerive(pass, row.pass_salt, alg.iter); }
    catch (e) { return bad('derive-failed', 'derive failed', 500); }
    if (!spEq(got, row.pass_hash)) {
      const f = await spNoteFail(env, user, sid, row, now);
      return { status: 403, idemDone: false, obj: { ok: false, storyPass: true, id: sid,
        error: 'パスコードが違います', errorCode: 'STORY_PASSCODE_INVALID',
        lockedOut: !!f.lockUntil, retryable: true, requestId } };
    }
    await spResetFail(env, user, sid);
    const lv = await spLockVersion(row);
    const s = await spSessionIssue(env, user, sid, lv);
    return { status: 200, idemDone: false, obj: { ok: true, storyPass: true, id: sid,
      storySession: s.token, exp: s.exp, requestId } };
  }

  if (op === 'storypassset') {
    if (!(SP_ITER > 0)) {
      return bad('STORY_PASSCODE_NOT_CONFIGURED',
        'passcode derivation parameters are not configured on this build', 409);
    }
    const next = spValidPass(body && body.pass);
    if (!next) return bad('bad-passcode', 'passcode must be ' + SP_PASS_MIN + '-' + SP_PASS_MAX + ' chars');
    const salt = spRandomHex(16);
    const alg  = spAlgString(SP_ITER);
    let hash = '';
    try { hash = await spDerive(next, salt, SP_ITER); }
    catch (e) { return bad('derive-failed', 'derive failed', 500); }

    if (!spIsLocked(row)) {
      /* ★初回設定: account auth のみで可。条件付き単一 UPDATE で race による上書きを防ぐ */
      const r = await env.DB.prepare(
        'UPDATE story_shadow SET pass_hash=?3, pass_salt=?4, pass_alg=?5, pass_updated_at=?6, ' +
        'pass_fail_count=0, pass_fail_window_at=NULL, pass_lock_until=NULL ' +
        'WHERE u=?1 AND story_id=?2 AND pass_hash IS NULL')
        .bind(user, sid, hash, salt, alg, now).run();
      const changed = (r && r.meta && +r.meta.changes) || 0;
      if (changed !== 1) return bad('STORY_PASSCODE_ALREADY_SET', 'passcode already set (race)', 409);
      return { status: 200, idemDone: false, obj: { ok: true, storyPass: true, id: sid, locked: true, created: true, requestId } };
    }
    /* ★変更: valid story session ＋ 現 lockVersion 一致が必須 */
    const v = await spSessionVerify(env, request, body, user, sid, row);
    if (!v.ok) return bad('STORY_LOCKED', 'unlock required to change the passcode', 409);
    const curLv = await spLockVersion(row);
    const r2 = await env.DB.prepare(
      'UPDATE story_shadow SET pass_hash=?3, pass_salt=?4, pass_alg=?5, pass_updated_at=?6, ' +
      'pass_fail_count=0, pass_fail_window_at=NULL, pass_lock_until=NULL ' +
      'WHERE u=?1 AND story_id=?2 AND pass_hash=?7 AND pass_updated_at=?8')
      .bind(user, sid, hash, salt, alg, now, row.pass_hash, row.pass_updated_at).run();
    const ch2 = (r2 && r2.meta && +r2.meta.changes) || 0;
    if (ch2 !== 1) return bad('STORY_PASSCODE_CAS_CONFLICT', 'lock state changed concurrently', 409);
    /* 旧 session は lockVersion mismatch で自動的に無効化される（KV は消さなくてよい） */
    return { status: 200, idemDone: false, obj: { ok: true, storyPass: true, id: sid, locked: true, changed: true,
      previousLockVersionRetired: !!curLv, requestId } };
  }

  if (op === 'storypassclear') {
    if (!spIsLocked(row)) return { status: 200, idemDone: false,
      obj: { ok: true, storyPass: true, id: sid, locked: false, alreadyClear: true, requestId } };
    const v = await spSessionVerify(env, request, body, user, sid, row);
    if (!v.ok) return bad('STORY_LOCKED', 'unlock required to clear the passcode', 409);
    const r = await env.DB.prepare(
      'UPDATE story_shadow SET pass_hash=NULL, pass_salt=NULL, pass_alg=NULL, pass_updated_at=NULL, ' +
      'pass_fail_count=NULL, pass_fail_window_at=NULL, pass_lock_until=NULL ' +
      'WHERE u=?1 AND story_id=?2 AND pass_hash=?3 AND pass_updated_at=?4')
      .bind(user, sid, row.pass_hash, row.pass_updated_at).run();
    const ch = (r && r.meta && +r.meta.changes) || 0;
    if (ch !== 1) return bad('STORY_PASSCODE_CAS_CONFLICT', 'lock state changed concurrently', 409);
    return { status: 200, idemDone: false, obj: { ok: true, storyPass: true, id: sid, locked: false, cleared: true, requestId } };
  }
  return bad('bad-request', 'unknown story passcode op');
}

/* protected op の入口ゲート。null を返したら通過、object を返したらそれが応答。 */
async function spGateProtected(env, request, user, op, sid, body, requestId) {
  if (!SP_PROTECTED_OPS[op]) return null;
  let row = null;
  try { row = await spLoadRow(env, user, sid); }
  catch (e) {
    return { status: 503, idemDone: false, obj: { ok: false, shadow: true, id: sid,
      error: 'lock state unavailable', errorCode: 'STORY_LOCK_STATE_UNAVAILABLE', retryable: true, requestId } };
  }
  if (!row || !spIsLocked(row)) return null;            // 未設定 story は完全に従来どおり
  const v = await spSessionVerify(env, request, body, user, sid, row);
  if (v.ok) return null;
  /* ★404 ではなく 409 STORY_LOCKED（fix705 は HOLD_CONTINUE / UNLOCK_REQUIRED と解釈する） */
  return { status: 409, idemDone: false, obj: { ok: false, shadow: true, id: sid, locked: true,
    error: 'この物語はパスコードで保護されています', errorCode: 'STORY_LOCKED',
    unlockRequired: true, reason: v.reason, retryable: false, requestId } };
}

async function handleStoryShadow(env, user, op, body, mid, requestId, spRequest) {
  const bad = (code, msg, status) => ({ status: status || 400, idemDone: false,
    obj: { ok: false, shadow: true, error: msg, errorCode: code, retryable: false, requestId } });
  if (op === 'listshadow') {
    /* ★STORY_PASSCODE: presentation 用に locked だけ足す。pass_hash / pass_salt / pass_alg は返さない。 */
    const rs = await env.DB.prepare('SELECT story_id, rev, updatedAt, title, turn_count, snippet, deleted, content_hash, authority, pass_hash FROM story_shadow WHERE u=?1 ORDER BY updatedAt DESC').bind(user).all();
    const rows = (rs && rs.results) || [];
    return { status: 200, idemDone: false, obj: { ok: true, shadow: true,
      stories: rows.map(r => ({ id: r.story_id, rev: +r.rev || 0, updatedAt: +r.updatedAt || 0,
        ...
        locked: !!(r.pass_hash != null && r.pass_hash !== '') })) } };
  }
  const sid = (body && body.id != null && body.id !== '') ? String(body.id).slice(0, 80) : '';
  if (!sid) return bad('bad-request', 'no id');
  /* ★★STORY_PASSCODE: protected op の唯一の enforcement point。
     未設定 story は null が返って完全に従来どおり通る。 */
  {
    const spGate = await spGateProtected(env, spRequest, user, op, sid, body, requestId);
    if (spGate) return spGate;
  }
  if (op === 'getstory') {
  ...
  if (op === 'putcanonical') {
    ...
    /* ★★v40.3 H-3: canonical 側も同じ guard。pcFail は retryable を立てないので
       ここでは直接 413 / non-retryable を返し、lock は release してから返す。 */
    if (typeof pBlobStr === 'string' && pBlobStr.length > CHR_STORY_BLOB_MAX_V403) {
      await pcRelease();
      return { status: 413, idemDone: false, obj: { ok: false, shadow: true, canonicalWrite: true, id: sid,
        error: 'この物語はクラウドに保存できる上限を超えています（' + pBlobStr.length + ' / ' + CHR_STORY_BLOB_MAX_V403 + '）。',
        errorCode: 'STORY_TOO_LARGE', limit: CHR_STORY_BLOB_MAX_V403, size: pBlobStr.length,
        retryable: false, requestId } };
    }
  ...
  /* ★★v40.3 H-3: D1 へ渡す前に落とす。non-retryable にして無限 retry を断つ。 */
  if (blobStr.length > CHR_STORY_BLOB_MAX_V403)
    return { status: 413, idemDone: false, obj: { ok: false, shadow: true, id: sid,
      error: 'この物語はクラウドに保存できる上限を超えています（' + blobStr.length + ' / ' + CHR_STORY_BLOB_MAX_V403 + '）。',
      errorCode: 'STORY_TOO_LARGE', limit: CHR_STORY_BLOB_MAX_V403, size: blobStr.length,
      retryable: false, requestId } };
