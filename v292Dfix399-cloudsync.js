...
(function(){
  ...
  function f821On(){
    ...
    try { if (localStorage.getItem('v292Dfix821On') === '1') return true; } catch(e){}
  ...
  function f821HasBody(id){ try { return localStorage.getItem('chr6_slot_' + id) != null; } catch(e){ return false; } }
  ...
  function collectLS(slotId){
    ...
    var ids;
    if (use821){
      /* membership source は body-backed。ただし caller が 1 本を要求しているなら広げない。 */
      ids = f605off() ? f821Eligible([slotId]) : f821PackageSlotIds(dead, null);
    } else {
      ids = f605off() ? [slotId] : liveSlotIds();
