  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 28, freeFallback: true, genBudget: gb23, inspect: true, inspectSpec: 'v20.5', avatarGuard: !!(cfg23 && cfg23.fix476Guard), avatarGuardSpec: 'v27.0', imgKeyMode: ((cfg23 && (cfg23.imgKeyMode === 'reserve' || cfg23.imgKeyMode === 'auto')) ? cfg23.imgKeyMode : 'primary'), paidKeySet: !!String(env.POLLINATIONS_KEY_PAID||'').trim(), lora420: true, debug420: true, style420: true, strict: String(env.ALLOW_IMAGE_FALLBACK||'') !== '1', d1: !!env.DB, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true,
        ...
        tombstoneGuard: true, workerBuild: 'v29', storyShadow: 1, imgStats: __imgStats, imgNoop: 1,
...
function chrCanonicalStoryContent(id, rec) {
  ...
  return {
    ...
    body: { cfg: (b.cfg === undefined ? null : b.cfg), cast: (b.cast === undefined ? null : b.cast),
    ...
    turnCount: (typeof r.turnCount === 'number' && r.turnCount >= 0) ? r.turnCount : turns.length,
    snippet: (r.snippet == null) ? '' : String(r.snippet).slice(0, 200)
  };
}
...
async function handleStoryShadow(env, user, op, body, mid, requestId) {
  ...
  if (op === 'listshadow') {
    const rs = await env.DB.prepare('SELECT story_id, rev, updatedAt, title, turn_count, snippet, deleted, content_hash FROM story_shadow WHERE u=?1 ORDER BY updatedAt DESC').bind(user).all();
    const rows = (rs && rs.results) || [];
  ...
  if (op === 'getstory') {
    const row = await env.DB.prepare('SELECT rev, updatedAt, blob, content_hash, title, turn_count, snippet, deleted FROM story_shadow WHERE u=?1 AND story_id=?2').bind(user, sid).first();
    ...
    return { status: 200, idemDone: false, obj: { ok: true, shadow: true, id: sid, rev: +row.rev || 0,
      updatedAt: +row.updatedAt || 0, serverHash: row.content_hash || null, deleted: !!row.deleted, record: rec } };
  }
  // ---- putstory ----
  ...
  const cur = await env.DB.prepare('SELECT rev, content_hash FROM story_shadow WHERE u=?1 AND story_id=?2').bind(user, sid).first();
  if (!cur) {
    if (baseRev !== 0) return conflict();                              // marker が進み過ぎ/表消失 → 409(serverRev:0)
    const ins = await env.DB.prepare("INSERT INTO story_shadow (u,story_id,rev,updatedAt,content_hash,blob,title,turn_count,snippet,deleted,device,build) VALUES (?1,?2,1,?3,?4,?5,?6,?7,?8,0,?9,?10) ON CONFLICT DO NOTHING").bind(user, sid, now, serverHash, blobStr, cc.title, cc.turnCount, cc.snippet, device, build).run();
  ...
  if (String(cur.content_hash || '') === serverHash) {
    return { status: 200, idemDone: true, obj: { ok: true, shadow: true, id: sid, rev: curRev, noop: true, serverHash: serverHash, requestId } };
  }
