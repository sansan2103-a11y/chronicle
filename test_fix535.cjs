...
console.log('\n== fix536b: 自己紹介を呼びかけと誤判定しない(30ターン実機で捕獲) ==');
{
  const n='少女は膝を抱えたまま、わずかに顎を引いた。\n名乗る声は小さい。';
  const r=run(n,[['シオン','シオンっていうんだ……たぶん']]);
  ok('★名乗りの台詞で本人が話者から外れない', r[0]==='シオン', r);
  const n2='アリアは首を振った。';
  const r2=run(n2,[['カエデ','アリア、それは違う']]);
  ok('呼びかけは従来どおり減点される(話者から外れる余地を残す)', r2[0]==='カエデ', r2);
}
