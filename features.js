          ...
          if (r.sys.indexOf('【反復の禁止') < 0){
            ...
            try {
              ...
              if (/続きを(?:自然に)?進めて/.test(_pt138) || /^続きを書/.test(_pt138)){
                ...
                _drama = '【続きを書く＝局面を動かす】前ターンの状態・感情・未完の動きを引き継いだ上で、状況そのものを次の段階へ動かす（場所転換／脱出や移動／新キャラ登場／状況急変／重要事実発覚／時間経過／関係性の変化 のいずれか）。同じ相手への加害や同じ膠着を細かく描き直すのは「前進」ではない。引き継ぎと前進は両立する（引き継いだ上で動かす）。';
