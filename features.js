...
(function v292Dfix96(){
  ...
  var BLOCK = [
    ...
    '',
    '【苦痛・損傷・恐怖の発声（最優先・抑制/幻想的指示より上位）】',
    '・痛み・損傷・強い恐怖を受けた瞬間、本人は必ずその場で声を上げる。「呻いた」等の要約は禁止し、発声をそのまま文字化（例:「ぎっ……！」「あ———……っ」「ぐ、ぅ……ひっ」。濁音・促音・長音で肉声の崩れを出す）。',
    '・声を主役に最低1つ書き（途切れ連続も可）、その後に喉・呼吸・震えの身体描写を短く添える。声の強度は刺激で段階を変え毎回変化させる。気絶・口を塞がれる等で物理的に不可能な時のみ省略可。'
  ].join('\n');
...
(function v292Dfix105(){
  ...
  var BLOCK = [
    ...
    '',
    '【場にいる者は“生きている”（自律発話・上位指示）】',
    '・場にいる全存在（NPC・怪異・人ならぬ者）は意志・欲求・恐怖を持ち、指定がなくても自分から考え喋り動く。毎ターン最低1〜2体が自発的にセリフか主体的行動を取る（先の「毎ターン喋らせる必要はない」より優先）。声を出せない状況の者のみ身体反応で示す。',
              ...
              if (_cast && Array.isArray(_cast.npcs)){
                ...
                if (_vl.length){
                  _voice = ['【登場キャラの声（場にいる者・性格に沿って喋る）】']
                    .concat(_vl)
                    .concat([
                      '・プレイヤー指定がなくても上記キャラや登場済みの怪異等は毎ターン最低1人が自分の意志で能動的に発言（呻きだけにしない）。発話不能な者は省く。主人公の新セリフは勝手に作らない。'
                    ]).join('\n');
                }
              }
            } catch(_ev){}
