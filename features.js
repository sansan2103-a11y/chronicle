...
(function v292Dfix96(){
  ...
  var BLOCK = [
    ...
    '',
    '【苦痛・損傷・恐怖の“発声”（最優先・他の文体/抑制/幻想的指示より上位）】',
...
(function v292Dfix105(){
  ...
  var BLOCK = [
    ...
    '',
    '【最重要・出力の鉄則（これだけは絶対に守る）】',
    '・このメッセージに書かれた全てのルール・指示は「あなたへの内部指示」である。物語の本文には絶対に出力してはならない。',
    '・「【重要ルール監査】」「物語進行:」「反応順序:」「五感拡充」「once」等、ルールの列挙・自己点検・チェックリスト・要約・メモを本文に書くことを固く禁じる。【】で囲んだ管理用の見出しブロックを本文に出してはならない。',
    '・出力は「物語の地の文」と「登場人物のセリフ（「」/ <say who=\"名前\">…</say>）」だけ。メタ的な説明、指示文の反復、英単語の混入（turn, once 等）をしない。',
    '・ルールはあくまで“どう書くか”の指針であり、“書く対象”ではない。読者には完成した物語だけが見える状態にすること。'
  ].join('\n');
          ...
          if (r.sys.indexOf('【反復の禁止') < 0){
            ...
            try {
              ...
              if (/続きを(?:自然に)?進めて/.test(_pt138) || /^続きを書/.test(_pt138)){
                ...
                _continueHint138 = '【続きの転換点と描写の濃さ】今ターン以下のいずれかを必ず起こす：(1)場所が変わる/移動 (2)新人物・存在・物が登場 (3)時間経過(数分後/朝/夕方/翌日) (4)状況急変(逃走・拘束変化・負傷・治療等) (5)重要事実が判明 (6)関係性が大きく変化。**直近3ターンと同じ種類の転換点を選ばない**（前回が(2)新キャラ登場なら今回は(1)場所転換や(3)時間経過にする等、同じパターン反復を厳禁。マンネリ化を絶対に避ける）。その瞬間を**主人公の五感**(視覚/聴覚/触覚/痛覚/嗅覚/内感)で**密度高く**描く。「〜寸前まで追い込まれる」「〜が忍び込んできた」のようなsummary風要約口調を禁止し、「今その瞬間に何が見え/聞こえ/触れ/匂うか」を具体動詞で描く。'
                  // v292Dfix153b(2026-05-30): 続きを書く連発で「前ターンと無関係に新展開が始まる」
                  //   （クレアが叩きつけられた直後に全員駆け出す等）を防ぐ。転換点を起こす前に、
                  //   直近3-5ターンの最終発言・最終動作・キャラの最終状態を最低1つ明示的に引き継ぐ。
                  + '【引き継いでから転換せよ】新しい転換点を起こす前に、直近3〜5ターンの「最終発言」「最終動作」「各キャラの最終状態」のうち最低1つを今ターン冒頭で明示的に引き継ぐ。引き継ぎなしに唐突な新展開を始めるのを禁止する（直前で負傷/拘束/掴まれた等の状態のキャラは、その状態を踏まえてから動かす）。';
              }
            } catch(_e138){}
            ...
            try {
              ...
              var _neoLines = [
                '【直前ターン連続性＝最強・最優先】今ターンを書く前に直近3〜5ターンを精読し、以下を必ず引き継ぐ。前ターンと無関係な新展開を唐突に始めない：',
                '・未完了の発言/問い：誰かの質問・呼びかけ・宣言がまだ処理されていなければ、応答するか意図的に避ける描写を入れる。宙に浮いた発言を放置しない。',
                '・物理状態の継続：負傷／拘束／移動／位置／死亡などは、変化や解決を描かない限り次ターンも保持する（負傷者を無傷で動かさない、死亡者を生存扱いしない、掴まれた者を脱出描写なしに自由にしない）。',
                '・感情の継続：直前で怖がる／怒る／疑う等の感情は急に消さない。変化させるなら必ず理由を描く。',
                '・進行中アクションの継続：「掴まれている」「咀嚼されている」「逃げている」等の継続状態は、完了または明確な変化が起きるまで描き続ける。途中で言及をやめない。',
