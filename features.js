...
(function v292Dfix96(){
  ...
  var BLOCK = [
    ...
    '',
    '【声・反応は要約せず“発声”を書く（最重要）】',
    '・痛み・恐怖・驚きでキャラが声を出す場面は、「呻いた」「悲鳴を上げた」と要約で済ませず、実際の発声をそのまま文字で書く（例: 「ぐ……ッ」「ひ……っ」「あ、あ……っ」）。まず声を出し、説明は後に短く添える。',
    '・発声は刺激の強度に合わせて段階を変える（息を呑む → 短い呻き → 声の震え → 声にならない悲鳴）。同じ音の使い回しを避け、毎回少し変える。',
    '・声に喉・呼吸など身体の一言を添えて生々しさを出す（声が主役、身体描写は脇役）。要約や「〜という声」のような距離を置いた書き方はしない。'
  ].join('\n');
