...
(function v292Dfix105(){
  ...
  var BLOCK = [
    ...
    '【展開を前に進める（進行エンジン・最優先）】',
    '・毎ターン状況を必ず一歩前進させ、前ターンと同じ動作・膠着を言い換えて描き直すのを禁じる。新しい展開ビートを最低1つ起こす（力関係の変化／新たな脅威・人物・情報／誰かの決断・行動／状況の悪化好転／場所・時間の移動 のいずれか）。',
    '・narrative の最後は余韻でなく次への変化のフックで終える（近づく足音、選択を迫る声、独りでに動く扉等）。空入力・「続きを進めて」のときこそ最も強く新展開を起こす。',
    '・文学的な描写の濃さ（Show Don\'t Tell・五感・心理）は保つ。目標は「濃いまま止まらず前へ」。',
    '',
    '【場にいる者は“生きている”】',
        ...
        if (r && typeof r.sys === 'string'){
          ...
          if (r.sys.indexOf('【反復の禁止') < 0){
            ...
            if (_lvl === 1){
              ...
              _drama = '【物語の推進=強め・最重要】毎ターン必ず新しい展開（転換点/新事実/急変/賭けの上昇/場面転換）を起こし、前ターンと同じ局面の足踏み・再描写を固く禁じる。停滞厳禁。プレイヤー入力は尊重し主人公は勝手に動かさない。';
            } else if (_lvl === 2){
            ...
            try {
              ...
              if (/続きを(?:自然に)?進めて/.test(_pt138) || /^続きを書/.test(_pt138)){
                ...
                _drama = '【物語の推進=最強・続きを書く強化】今ターンは必ず新しい大きな展開を1つ起こす（場所転換／新キャラ登場／状況急変／重要事実発覚／時間経過／関係性の変化 のいずれか）。前場面を淡々と続けず明確に一段階前へ。停滞・微小変化のみ厳禁。';
                _continueHint138 = '【続きの転換点】今ターン以下を必ず1つ：(1)場所/移動 (2)新人物・物の登場 (3)時間経過 (4)状況急変(逃走/拘束変化/負傷/治療) (5)重要事実判明 (6)関係性の変化。直近3ターンと同じ種類の転換点を選ばずマンネリ厳禁。その瞬間を主人公の五感で密度高く描き、「〜寸前まで追い込まれる」等のsummary風要約を禁止し具体動詞で。ただし転換の前に、直近3〜5ターンの最終発言/動作/各キャラの最終状態の最低1つを今ターン冒頭で明示的に引き継いでから転換する（負傷/拘束等の状態のキャラはその状態を踏まえてから動かす。引き継ぎなしの唐突な新展開は禁止）。';
              }
            } catch(_e138){}
            ...
            try {
              ...
              // v292Dfix161: 軽量化圧縮（趣旨保持）。
              var _neoLines = [
                '【直前ターン連続性＝最優先】今ターン前に直近3〜5ターンを精読し必ず引き継ぐ（前ターンと無関係な新展開を唐突に始めない）：未完了の発言/問いは応答か意図的回避を描く／負傷・拘束・移動・位置・死亡などの物理状態は変化を描かない限り保持（負傷者を無傷で動かさない・死者を生存扱いしない・掴まれた者を脱出描写なしに自由にしない）／感情は急に消さず変化なら理由を描く／「掴まれている」等の進行中アクションは完了・変化まで描き続ける／直前の最後の発言・動作に反応する形で始める。'
              ];
