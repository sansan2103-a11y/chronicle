...
(function bootstrap(){
  ...
  (function metaStrip159(){
    ...
    function isPureMeta(s){
      ...
      if (/^【[^】]*(?:反応|発声|セリフ|内心|状態)[^】]*】$/.test(s.trim())) return true;   // v292Dfix179: 行全体が「【…の反応】」等の見出しの保険ネット
      // v292Dfix183: 自己採点・出力チェックリストの漏れ（モデルが優先順位ツリー等を真似て本文に書く）。
      if (/^【[^】]*(?:チェック|確認|点検|自己採点|採点|優先順位|判断の優先)[^】]*】$/.test(s.trim())) return true;   // 「【出力チェックリスト】」等の見出し
...
(function v292Dfix105(){
  ...
  var BLOCK = [
    ...
    '',
    '【描写は具体的に・筋を通す】',
    '・比喩や美文より「誰の身に何が起きているか」の具体と物理的な筋を優先する。比喩で意味が曖昧になるなら使わない。長さを埋めること自体は目的にしない（短くても筋が通って具体的なら良い）。',
    '・物理的に破綻した描写を出さない（無生物が泣く/血を流す、左右や位置・人物・負傷が前ターンと食い違う、痛みの主体がすり替わる等）。位置・負傷・所持・誰の行動かは前ターンと一貫させる。',
    '・直接の説明を避けるのは良いが、何が起きたか・誰がどうしたかは読んで明確に分かるように保つ。抽象的で思わせぶりなだけの内心（「解放すべき結論」等）で埋めない。',
    '',
    '【一読で伝わる日本語（厳守）】',
    '・文学性とは凝った言い回しのことではなく「一読で情景が浮かぶこと」。読み返さないと意味が取れない文・不自然な言い回しは、たとえ詩的でも失敗。明晰さを優先する。',
    '・無理な使役/受身を重ねない（「浮き上がらせて」「湧き上げさせる」等）。基本は主語がする普通の動詞で書く（浮かぶ/差す/震える/開く）。',
    '・主語と述語が実際に噛み合う組み合わせにする。比喩でも「月光が瞼を貫く」「冷気が瞼からこぼれる」「空気が鼓膜を伝って耳底に染み渡る」のような物理的に起きない主語-動詞は避け、起きていることをそのまま書く。',
    '・各文は述語で言い切る。「〜浮き上がらせて。」のような連用形・て止めの断片で文を終えない。',
    '・一文に修飾・比喩・節を詰め込みすぎない。主語と述語を近づけ、原則1文1情報。凝るより通じるを優先する。',
    '',
    '【出力の鉄則】',
