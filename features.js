...
(function bootstrap(){
  ...
  (function verbatim(){
    var VERBATIM_TEXT = [
      ...
      '',
      'narrative は、プレイヤー入力で起きること（入力が「続きを自然に進めて」等の"継続の合図"なら、直前ターンの状態・感情・未完の動きを引き継いだ上での【次の展開】）を今ターンの起点にする。',
      '直前ターン(recentScenes)の本文を再演・コピー・言い換えして書き始めてはならない。継続の合図のときも同じ場面・同じ文を書き直さず、状況を必ず次の段階へ前進させる。',
      '━━━━━━━━━━━━━━━━━━━━━━━━━'
    ].join('\n');
  ...
  (function reactionVoice157(){
    ...
    // v292Dfix164: 「声」フィールドに状態描写(地の文)が入る件の検出。実際の発話/叫びでなく
    //   ナレーション的な状態記述（〜震える/〜身悶える、自分の無力さ 等の三人称的描写）は会話ログに
    //   セリフとして出さず内心(反応)へ回す。短い叫び・命乞いは誤判定しないよう長さで除外。
    function looksStateDescription(t){
      t = String(t || '').trim().replace(/[。、！？…\s]+$/,'');
      if (!t || t.length < 12) return false;   // 短い叫び・命乞いは実発話として残す
    ...
    function register(){
      ...
      Planner._parseExtensions.push(function reactionVoiceExt(plan, ctx){
        try {
          ...
          if (_cut >= 0){
            _joined = _joined.slice(0, _cut);
          }
          // v292Dfix179: モデルが反応セクションに「【ミリアの反応】」「【…本能的反応】」等の
          //   見出しを付け、それが <react> マーカーより前にあると上のカットで残ってしまう
          //   (プロンプトが【…】見出しだらけなのを真似て出力に書く)。本文末尾にぶら下がる
          //   反応系の見出しを掃除する。地の文(文学的散文)は【】見出しで終わらないので安全。
          if (_cut >= 0 || /【[^】]*(?:反応|発声)[^】]*】\s*$/.test(_joined)){
            _joined = _joined.replace(/[\s　]*【[^】]{0,24}(?:反応|発声|セリフ|声|内心|状態|心)[^】]{0,12}】[\s　]*$/,'');
          }
          plan.narrative = _joined.split('\n')
            .map(function(l){ return String(l).trim(); })
            .filter(function(l){ return l && l.length > 0; });
          // 行全体が【…】見出しだけの行が末尾に残っていれば除去(地の文ではありえない)。
          while (plan.narrative.length &&
                 /^【[^】]*】$/.test(plan.narrative[plan.narrative.length - 1])){
            plan.narrative.pop();
          }
          ...
          while ((m = rx.exec(raw)) !== null){
            ...
            // v292Dfix164: 「声」に状態描写が入っていたら発話扱いにせず反応(内心)へ降格＝会話ログに出さない。
  ...
  (function metaStrip159(){
    ...
    // v292Dfix159b(2026-05-30): AI作業報告・自己言及メタ。今日プロンプト指示を増やした副作用で
    //   モデルが「〜に基づいて創作しています／修正箇所をご確認ください／調整いたしました」のように
    //   システム指示へ"返答"する作業報告を本文に漏らす。これらは確実にメタなので行ごと除去。
    var WORK_REPORT = /に基づいて(?:創作|執筆|生成|作成)|修正箇所|調整(?:いた)?しました|あらためて調整|プレイヤー(?:の)?入力「|再度ご確認ください|ご確認ください|創作して(?:います|おります)/;
    function isPureMeta(s){
      ...
      // v292Dfix184: 行全体が「【…】」見出しだけの行は、地の文(文学的散文)には絶対に現れない
      //   ＝モデルが発明した構造/メタ見出し(【出力チェックリスト】【ミクロな時間軸で描写】【…の反応】等)
      //   の漏れ。種類を問わず一律ドロップ(キーワードのモグラ叩きをやめ一般化)。
      if (/^【[^】]{0,40}】$/.test(s.trim())) return true;
      // v292Dfix183: 自己採点・チェックリスト項目の漏れ（優先順位ツリー等を真似て本文に書く）。
      if (/^[-・*●○]?\s*\[[ xX✓○●　]\]/.test(s.trim())) return true;   // 「- [x] …」「・[ ] …」形式のチェック項目
      if (/^[①②③④⑤⑥⑦⑧⑨]\s*(?:破綻|連続性|前進|文体|満た|チェック|確認|OK|済)/.test(s.trim())) return true;   // ①②③ の優先順位ツリーを自己確認として復唱した行
      // v292Dfix186b: 「『…』という直近の入力」「プレイヤー入力を踏まえ…」型の入力メタ参照の漏れ
      //   （モデルが本文末等で入力/指示を引用してメタ的に言及する）。地の文は入力を参照しない。
      if (/という(?:直近の|今回の)?(?:入力|プレイヤー入力|指示|展開指示)/.test(s)) return true;
      if (/プレイヤー(?:の)?入力(?:を|に|の|は)/.test(s)) return true;
      return false;
    }
    function register(){
      if (typeof Planner !== 'object' || !Planner || !Array.isArray(Planner._parseExtensions)) return false;
  ...
  (function npcHumanity190(){
    ...
    var EMIT = [
      ...
      '<state> には からだ/こころ/本能 に加え、次の永続フィールドを使う。「からだ」は"今この瞬間の様子"（毎ターン変わる）、下の3つは"消えない記録"。混同せず使い分ける:',
      '傷="負傷・損傷・精神的ショックを受けた瞬間に【必ず】記入。部位・重症度・原因を簡潔に。例: 左眼球摘出(重傷)/妖怪/未治療。からだに書くだけで済ませず傷にも残す。一度書いたら治療されるまで毎ターン同じ傷を保持する"',
      '関係="主人公や他キャラへの今の関係・信頼/警戒。例: カエデ:命を握られ警戒 ／ ミリア:庇いたい"',
      '未解決="抱えている感情の負債。例: ミリアを救えなかった負い目"',
      '・傷/関係/未解決は【永続】。治療・和解・解消などの出来事が起きるまで毎ターン保持し、勝手に消さない。中身が空の属性（傷=""等）は書かず、内容がある時だけ付ける。',
      '',
      '【NPCは人間らしく会話する】',
      '・NPCは説明対象でなく、生きた人物。状況が許せば悲鳴・反応だけで終わらせず、目的・関係・感情に沿って【言葉で話す】（問いかけ・要求・拒絶・取引・告白・皮肉・弱音・強がり等）。',
      '・同じ場にいるNPC同士も、入力を待たず自然に会話・衝突・協力・沈黙する（2〜4往復程度で関係や次の行動が見えるように）。主人公にも自分から話しかける（助けを乞う・詰る・確認する・情報を伝える等）。',
      '・ただし型に嵌めない。激痛・強い恐怖の極限では言葉にならず声が途切れるのも自然。毎ターン全員を喋らせる必要はなく、その場・その人物に必要なやり取りだけでよい。'
    ].join('\n');
    function buildBlock(){
      try {
...
(function v292Dfix49(){
  ...
  (function(){
    ...
    var HOOK_BLOCK = [
      ...
      '・例:',
      '    - 「相手の肩が震えている」(→ DO「肩を抱く」を誘導)',
      '    - 「相手は何か言いたそうにしている」(→ SAY「どうしたの?」を誘導)',
...
(function v292Dfix96(){
  ...
  var MARK = '【セリフと物音の区別';
  var BLOCK = [
    // v292Dfix161: 軽量化。冗長な重複を圧縮（趣旨は保持）。
    '【セリフと物音の区別（会話ログ用・厳守）】',
    '・「」/<say> は登場人物が口に出した言葉だけに使う。物音・効果音・気配（ピタ/ガタッ/コツコツ/ヒタヒタ等）は地の文で書き、「」/<say>で囲まない。',
    '・悲鳴・うめき・荒い息（きゃあっ/うっ/ひっ）はセリフなので「」/<say>でよい。怪異やモンスターが言葉を発する場合は who にその名前を付けてセリフにしてよい。',
    '',
    '【苦痛・損傷・恐怖の発声】',
    '・痛み・損傷・強い恐怖の瞬間は、考える前に身体が反応する。声が漏れる場面ならそのまま文字化してよい（「ぎっ……！」「あ———……っ」等、濁音・促音・長音で肉声の崩れを）。',
    '・ただし毎回絶叫させる必要はない。呻き・押し殺した息・硬直・無言など、そのキャラと場面に合った出方を選び、同じ叫びを繰り返さない。生の声と言葉での発話を場面に応じて混ぜる。'
  ].join('\n');
...
(function v292Dfix105(){
  ...
  var BLOCK = [
    ...
    '・描写の濃さ（五感・心理・具体）は保つ。ただし描写を重ねることが目的化して話が止まらないように。「濃いまま止まらず前へ」。',
    '',
    '【場にいる者は“生きている”】',
    ...
    '④文体：①〜③を満たした上で、五感と心理のある濃い描写にする。ただし一読で伝わることを優先し、凝った言い回しで②③を犠牲にしない。',
    ...
    P.build = function(){
      ...
      try {
        if (r && typeof r.sys === 'string'){
          // v292Dfix188: プロンプト大改修（おしん指示）。質感ガイドのShow-not-tell禁止リスト＋
          //   機械的Staging（Stage0-3）＋冗長な反応/前進ブロックを最終sysから除去する。
          //   これらは(a)purple proseの元凶(禁止リスト)(b)負傷描写の単調化(機械Staging)
          //   (c)tree/進行エンジンと重複(場面前進)。代わりに下の BLOCK の【描写の芯】が
          //   具体描写を"肯定形＋型に嵌めない"で担い、優先順位ツリーが順位を担う。
          //   repoの土台ファイルは無傷＝この後処理を外せば元通り（可逆）。
          [
            /【物語の質感ガイド[\s\S]*?(?=【|$)/,
            /【損傷・激痛時の反応[\s\S]*?(?=【|$)/,
            /【反応の強度スケール[\s\S]*?(?=【|$)/,
            /【場面前進と反応の鮮度[\s\S]*?(?=【|$)/,
            /【苦痛・損傷・恐怖の発声[\s\S]*?(?=【|$)/
          ].forEach(function(_re){ r.sys = r.sys.replace(_re, ''); });
          ...
          if (r.sys.indexOf('【反復の禁止') < 0){
            var _rep = '【反復の禁止】直近ターンの文・言い回し・同じ動作を繰り返さない。同じ対象に同じ行為を再描写しない。毎ターン新しい言葉と絵で書く。';
            var _lvl = 2;
            ...
            if (_lvl === 1){
              ...
              _drama = '【物語の推進＝強め】前の局面を引き継いだ上で、毎ターン状況を次の段階へ動かす（転換点/新事実/急変/賭けの上昇/場面転換）。同じ局面の足踏みや、同じ相手への加害・同じ行為を細かく描き直すだけにしない。プレイヤー入力は尊重し主人公は勝手に動かさない。';
            } else if (_lvl === 2){
              _drama = '【物語の推進＝標準】前の局面を引き継いだ上で、状況を一歩前へ動かす（場所・状況・登場人物・時間のどれかが変わる）。同じ局面の足踏みや同じ行為の再描写だけで終えない。';
            }
            ...
            try {
              ...
              if (/続きを(?:自然に)?進めて/.test(_pt138) || /^続きを書/.test(_pt138)){
                // v292Dfix161: 軽量化圧縮（fix138/153b 趣旨保持）。
                // v292Dfix191(2026-06-02): 「続きを書く」の進み幅を📖進行セレクタ(dramaLevel)に連動。
                //   弱め(=①小さく一歩) / 標準 / 強め(=②大胆に転がす)。"横に広げる"原則は全レベル共通で、
                //   進行レベルが変えるのは「持ち込む新要素の数」と「動かす勢い」だけ。ユーザーは進行
                //   セレクタ一つで①↔②を切り替えられる。
                var _横原則 = '重要な原則："縦"（同じ行為・同じ被害の強度を上げる＝グロや痛みを激しくするだけ）は前進ではない。"横"＝新しい要素（新キャラや存在の登場／場所の移動・転換／脱出や追跡／時間経過／重要な事実・謎の発覚／力関係や関係性の変化／思いがけない選択肢や申し出）で世界を広げること。主人公は受け身の観察に留めず、状況に応じて自分から動く（抵抗・逃走・対峙・利用・取引・救出・探索・呼びかけ等。冷静なキャラなら計算ずくの選択として）。同じ被害描写の反復で終えない。';
                var _幅;
                if (_lvl <= 1){
                  // ① 小さく一歩（弱め）
                  _幅 = '前ターンを丁寧に引き継いだ上で、新しい要素を1つだけ、控えめに小さく持ち込む（1ビート）。一気に話を飛ばさず、プレイヤーが次の大きな選択をしやすいよう半歩〜一歩だけ進める。';
                } else if (_lvl >= 3){
                  // ② 大胆に転がす（強め）
                  _幅 = '前ターンを引き継いだ上で、新しい要素を2つ以上持ち込み、複数の展開を一気に動かして話を大胆に転がす（場面転換＋新事実、追跡＋関係の急変などを組み合わせてよい）。足踏みは厳禁。';
                } else {
                  // 標準
                  _幅 = '前ターンを引き継いだ上で、新しい要素を1つ持ち込み、状況を一段階はっきり進める。';
                }
                _drama = '【続きを書く＝物語を"横"に広げる】' + _幅 + ' ' + _横原則;
                _continueHint138 = '【続きの転換点】前ターン末の発言/動作/各キャラの状態（負傷・拘束等）を今ターン冒頭で引き継いでから、状況を次へ動かす。動かし方の例：場所/移動・新人物や物の登場・時間経過・状況急変（逃走/拘束変化/治療/別の脅威の出現）・重要事実判明・関係性の変化。直近数ターンと同じ種類の転換は避ける。起きる出来事を主人公の五感で具体的に描き、「〜寸前まで追い込まれる」のような要約で済ませない。';
              }
            } catch(_e138){}
            ...
            if (_rl <= 0){
              ...
              _react = '【反応=濃いめ】場の各キャラを即座に濃く反応させる（感情＋身体反応＋声を重ね、要約で流さない。性格ごとに違う反応）。';
            } else {
              _react = ['【キャラの反応（標準）】',
            ...
            if (_dl <= 0){
              ...
              _dlg = '【セリフ=濃いめ】性格・口調から生まれる固有の言い回しで、呻きだけでなく中身ある言葉（問い/拒絶/懇願/皮肉/独白）を最低1つ。言外の含みを込める。発話不能な者のみ省く。';
            } else {
              _dlg = ['【セリフ（標準）】',
            ...
            try {
              ...
              if (_cast && Array.isArray(_cast.npcs)){
                ...
                if (_vl.length){
                  _voice = ['【登場キャラの声（場にいる者・性格に沿って喋る）】']
                    .concat(_vl)
                    .concat([
                      '・上記キャラや登場済みの怪異等は、場面が促せば自分の意志で発言する（呻きだけでなく言葉も交える）。全員が沈黙するのが自然な場面では無理に喋らせない。主人公の新セリフは勝手に作らない。'
                    ]).join('\n');
                }
              }
            } catch(_ev){}
            ...
            var _banter = ['【掛け合い・テンポ（会話を弾ませる）】',
              '・会話が自然に弾む場面では、複数人の短い往復で掛け合いを作る（数のノルマではない。緊迫・沈黙・1対1が活きる場面はそれを優先）。',
              '・セリフを交わすときは直前の誰かの発言や行動に噛み合わせる（問い→応答、挑発→反発、提案→同意/拒否）。独白の羅列にしない。',
              '・掛け合いが起きる場面では、短い往復で会話を前へ転がし、次の発言や行動を誘う一言・問いで締めると弾みやすい。'].join('\n');
            // v292Dfix131: 描写改善（圧縮版・fix115教訓で1行に詰めて sys 膨張回避）。
            var _depict = '【描写・没入】STORY/DO指示の事象は冒頭1-2文で具体動詞で直接描く（結果のみ禁）。主人公の五感（視/聴/触/痛/嗅/内感）を毎ターン最低1つ混ぜる。婉曲（〜が起きる/〜があった/〜のような声/〜が漏れた）禁・具体動詞優先（突き刺す/引き抜く/押し込む/噛み砕く/滴る）。短文連発禁・動作→感覚→反応の流れで繋ぐ。';
            // v292Dfix134: Few-shot 4例（グロ/心理/移動/官能）を sys に同梱。モデルが
            // ルール羅列より「お手本」を真似る傾向を利用して、比喩癖・婉曲癖・神視点を
            // 根本から矯正する。例文中の "主人公"/"相手" は cast 名で動的置換し具体感UP。
            // 例は3-5文に圧縮、合計sys+700字程度（fix115閾値9282以下を維持）。
            var _hN134 = '主人公', _rN134 = '相手';
            try {
              var _stF = null;
              try { if (typeof S !== 'undefined' && S) _stF = S; } catch(_es134){}
              if (!_stF) _stF = window.S || null;
              if (_stF && _stF.cast){
                if (_stF.cast.hero && _stF.cast.hero.name) _hN134 = _stF.cast.hero.name;
            ...
            // v292Dfix153a(2026-05-30): 直前ターン連続性=最強モード。おしん指摘「展開が前と繋がらない」
            //   + Claude実機調査で判明した3大課題（[A]キャラ消失 [C]状態の漸進的劣化 + 物理状態/進行中
            //   アクションの切り捨て）への対策。fix132 _continuity は「矛盾するな」程度で弱かったので、
            //   直近3-5ターンの精読・キャラ点呼（消失禁止）・状態詳細度維持・進行中アクション継続・
            //   既登場キャラの「???」再導入禁止 を明示する強いブロックを追加する。
            //   キャラ点呼は直近3ターンの narrative に名前が出た cast を動的に列挙し具体化する。
            //   このブロックは fix150 緊急圧縮の削減対象に含めない（連続性は最優先で常時保持）。
            var _continuityNeo = '';
            try {
              var _ssN = null;
              try { if (typeof S !== 'undefined' && S) _ssN = S; } catch(_esN){}
              if (!_ssN) _ssN = window.S || null;
              var _rollNames = [];
              if (_ssN && _ssN.cast){
                var _allN = [];
                if (_ssN.cast.hero && _ssN.cast.hero.name) _allN.push(_ssN.cast.hero.name);
                if (Array.isArray(_ssN.cast.npcs)) _ssN.cast.npcs.forEach(function(n){ if (n && n.name) _allN.push(n.name); });
                _allN = _allN.filter(function(n, i, a){ return n && a.indexOf(n) === i; });
                var _t3 = (_ssN.turns || []).slice(-3);
                var _txt3 = _t3.map(function(t){ return (t && t.narrative) || ''; }).join('\n');
                _allN.forEach(function(nm){ if (_txt3.indexOf(nm) >= 0) _rollNames.push(nm); });
              }
              // v292Dfix161: 軽量化圧縮（趣旨保持）。
              var _neoLines = [
                '【直前ターン連続性】今ターン前に直近3〜5ターンを精読し引き継ぐ。ただし引き継ぎは停滞ではない＝引き継いだ上で状況は前へ動かす（今の局面を次の段階へ進めるのは前進であり推奨、前ターンと無関係な展開を唐突に始めるのだけ避ける）：未完了の発言/問いは応答か意図的回避を描く／負傷・拘束・移動・位置・死亡などの物理状態は変化を描かない限り保持（負傷者を無傷で動かさない・死者を生存扱いしない・掴まれた者を脱出描写なしに自由にしない）／感情は急に消さず変化なら理由を描く／「掴まれている」等の進行中アクションは完了・変化まで描き続ける／直前の最後の発言・動作に反応する形で始める。'
              ];
              if (_rollNames.length){
                _neoLines.push('【キャラ点呼＝消失禁止】直近2〜3ターンに登場した次のキャラを今ターンで最低1回、内部メモ(体・心・本能＝現在状態)を踏まえ状態描写する（沈黙・気絶でも「今どうしているか」を示す）：' + _rollNames.join('、') + '。一人も取りこぼさない。状態の詳細度は前ターン同等以上を保ち漸進的フェードアウト禁止。既登場・命名済みキャラを「???」「謎の少女」等の新キャラ扱いで再登場させず必ず名前で扱う。');
              } else {
                _neoLines.push('【キャラ点呼＝消失禁止】直近2〜3ターンに登場した全キャラを今ターンで最低1回、内部メモの体・心・本能を踏まえ描写（消失・無言フェードアウト禁止）。詳細度は前ターン同等以上、既登場キャラを「???」等の新キャラ扱いで再導入しない。');
              }
              _continuityNeo = _neoLines.join('\n');
            } catch(_eNeo){}
            ...
            var _reactVoice = ['【キャラの反応＝本文の後に必ず出力（会話ログ用）】',
              ...
              '・「声」は"反応として口から漏れたもの"であり整った独白ではない。「後悔」「選択」「存在」のような抽象的な文や、「〜身体が震える」「〜苦痛に身悶える」「自分の無力さ」のような状態描写（地の文）は声に書かず「反応」へ。声は実際に口に出した言葉・叫び・呻きだけ、短く生々しく途切れてよい。',
              '・恐怖・激痛・絶望の瞬間は、考える前に身体が反応する。声・呻き・息の乱れ・硬直など、そのキャラ固有の出方で表し、毎回同じ絶叫にしない。場面によっては短い言葉や沈黙が自然なこともある。',
              '・場にいて反応しうるキャラは全員ぶん（1人1行）。<react>は本文に混ぜず後にまとめる。主人公の声はプレイヤー入力と矛盾しない範囲に。',
              '・反応の前に「【ミリアの反応】」「【…の反応】」のような見出し・ラベルを書かない。本文の直後に <react .../> タグだけを並べる（見出し行は出力しない）。'].join('\n');
            ...
            var _speakerExc = ['【話者ラベルの例外＝最優先（先の「who は登録名のみ」より優先）】',
              'セリフの話者(<say who="…">)は原則 登録キャラ（' +
                (function(){ try { var c=(typeof S!=='undefined'&&S.cast)?S.cast:(window.S&&window.S.cast); var ns=[]; if(c){ if(c.hero&&c.hero.name)ns.push(c.hero.name); if(Array.isArray(c.npcs))c.npcs.forEach(function(n){if(n&&n.name)ns.push(n.name);}); } return ns.join('、'); } catch(e){ return '登録キャラ'; } })() + '）の名前を使う。ただし登録キャラに居ない"名無しの存在"（怪物・異形・正体不明の声・まだ名前のない人物）が喋るときは、人間キャラの名前を絶対に当てず、その存在の呼称（「異形」「謎の声」「何か」「？」）を who に使う。文脈から誰のセリフかを判断し、登場人物でない存在のセリフを人物（特に主人公）に割り当てない。'].join('\n');
            // v292Dfix161: 軽量化圧縮（趣旨保持）。fix155②/fix159 の要点を統合。
