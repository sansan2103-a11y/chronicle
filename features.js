          ...
          if (r.sys.indexOf('【反復の禁止') < 0){
            ...
            try {
              ...
              if (/続きを(?:自然に)?進めて/.test(_pt138) || /^続きを書/.test(_pt138)){
                ...
                _drama = '【続きを書く＝局面を動かす】前ターンの状態・感情・未完の動きを引き継いだ上で、状況そのものを次の段階へ動かす（場所転換／脱出や移動／新キャラ登場／状況急変／重要事実発覚／時間経過／関係性の変化 のいずれか）。同じ相手への加害や同じ膠着を細かく描き直すのは「前進」ではない。引き継ぎと前進は両立する（引き継いだ上で動かす）。重要：「続きを書く」はプレイヤーが次の展開（主人公の行動を含む）をAIに委ねた合図。主人公が状況に応じて自分から動く（抵抗・逃走・対峙・救出・探索・呼びかけ等）ことを許容し、受け身の観察だけに留めない。同じ被害描写を繰り返すのでなく、主人公か場面を必ず一歩動かす。';
