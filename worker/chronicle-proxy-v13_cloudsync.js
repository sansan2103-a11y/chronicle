...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 13, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true }, 200, request);
    }
    ...
    // ---------- ★v10: /save : クロスデバイス・セーブ同期(KV保存) ----------
    if (path === '/save') {
      if (!env.LEDGER) return json({ error: 'LEDGER(KV)が未バインドです' }, 503, request);
      const skey = 'save:' + gate.codeKey;      // ユーザー一意(メール or 合言葉)ごとの金庫
      const op = body && body.op;
      if (op === 'meta') {
        const m = await env.LEDGER.get(skey + ':meta');
        const ns = await ensureNs(env, gate.codeKey);   // ★v11: 画像配信用の名前空間トークン(推測不能)
        return json({ ok: true, meta: m ? JSON.parse(m) : null, ns }, 200, request);
      }
      if (op === 'get') {
        const raw = await env.LEDGER.get(skey);
        return json({ ok: true, data: raw ? JSON.parse(raw) : null }, 200, request);
      }
      if (op === 'put') {
        const pkg = body.pkg;
        if (!pkg || typeof pkg !== 'object') return json({ error: 'no pkg' }, 400, request);
        // ★v12: light push(pkg.idb無し)でも既存blobのidb(画像)を消さないよう温存する。
        //   v11で自動同期がlight化した結果、次のlight pushがidb無しでblobを上書き→画像喪失→/img 404 になっていた根治。
        if (!pkg.idb || typeof pkg.idb !== 'object' || !Object.keys(pkg.idb).length) {
        ...
        if (str.length > 24 * 1024 * 1024) return json({ error: 'セーブが大きすぎます(24MB超)' }, 413, request); // KV値上限25MiBの手前
        await env.LEDGER.put(skey, str);          // 本体
        await env.LEDGER.put(skey + ':meta', JSON.stringify({ updatedAt: +pkg.updatedAt || Date.now(), device: String(pkg.device || ''), size: str.length })); // メタ(別キー=1write/sec制約OK)
        const ns = await ensureNs(env, gate.codeKey);   // ★v11
        // ★v11: 画像を個別キーに分割保存(配信用)。fullpush(pkg.idbあり)の時だけ。best-effort・非ブロッキング。
        if (pkg.idb && typeof pkg.idb === 'object') {
          ctx.waitUntil((async () => {
            for (const kk of Object.keys(pkg.idb)) {
              try { await env.LEDGER.put('img:' + ns + ':' + kk, String(pkg.idb[kk])); } catch (e) {}
            }
          })());
        }
        return json({ ok: true, size: str.length, ns }, 200, request);
      }
      // ★v13: 画像1枚だけ更新(split key直書き)。フル送信(数MB)が失敗する環境向けの軽量更新口。
      if (op === 'putimg') {
        ...
        const ns = await ensureNs(env, gate.codeKey);
        await env.LEDGER.put('img:' + ns + ':' + k, data);
        return json({ ok: true, ns: ns, k: k, size: data.length }, 200, request);
      }
      return json({ error: 'unknown save op (get|put|meta|putimg)' }, 400, request);
    }
