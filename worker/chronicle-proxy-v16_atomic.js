...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 16, d1: !!env.DB, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true }, 200, request);
    }
...
async function handleSave(request, env, ctx) {
  ...
  try {
    ...
    if (op === 'meta') {
      ...
      return json({ ok: true, meta, rev: meta ? (+meta.rev || 0) : 0, ns, v: 16, d1: !!d1, requestId }, 200, request);
    }
    ...
    if (op === 'put' || op === 'forceput') {
      ...
      if (d1) {
        ...
        // ★楽観分岐(fork): baseRevが現行より古い=別デバイスが先に進めた。相手を消さず両保持。
        if (op === 'put' && hasBase && cur && baseRev < curRev) {
          return await saveIncomingAsFork(request, env, ctx, { user, pkg, str, baseRev, curRev, curUpdatedAt: (+cur.updatedAt || 0), curDevice: String(cur.device || ''), ns, requestId });
        }

        if (op === 'forceput') {
          // ★B-1: 退避INSERT + 本体UPSERT を1バッチでアトミックに(黙って消さない)
          const stmts = [];
          if (cur && cur.blob) {
            const bkind = 'fork:' + String(cur.device || 'dev').replace(/[^\w\-\.\(\)]/g, '').slice(0, 24) + ':' + (+cur.updatedAt || Date.now());
            stmts.push(env.DB.prepare('INSERT OR REPLACE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)')
              .bind(user, bkind, curRev, +cur.baseRev || 0, +cur.updatedAt || 0, String(cur.device || ''), +cur.size || 0, cur.blob));
          }
          const newRev = curRev + 1;
          stmts.push(env.DB.prepare('INSERT OR REPLACE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)')
            .bind(user, 'main', newRev, hasBase ? baseRev : curRev, now, dev, str.length, str));
          await env.DB.batch(stmts);
          ctx.waitUntil(trimForks(env, user));
          return json({ ok: true, rev: newRev, size: str.length, ns, requestId }, 200, request);
        }
        ...
        if (cur) {
          ...
          if (!d1Changed(upd)) {
            ...
            return await saveIncomingAsFork(request, env, ctx, { user, pkg, str, baseRev: newBase, curRev: latest ? (+latest.rev || 0) : curRev, curUpdatedAt: latest ? (+latest.updatedAt || 0) : 0, curDevice: latest ? String(latest.device || '') : '', ns, requestId });
          }
          return json({ ok: true, rev: d1FirstRev(upd, curRev + 1), size: str.length, ns, requestId }, 200, request);
        } else {
          ...
          if (!d1Changed(ins)) {
            ...
            const latest = await env.DB.prepare('SELECT rev, updatedAt, device FROM saves WHERE u=?1 AND kind=?2').bind(user, 'main').first();
            return await saveIncomingAsFork(request, env, ctx, { user, pkg, str, baseRev: newBase, curRev: latest ? (+latest.rev || 0) : 0, curUpdatedAt: latest ? (+latest.updatedAt || 0) : 0, curDevice: latest ? String(latest.device || '') : '', ns, requestId });
          }
          return json({ ok: true, rev: 1, size: str.length, ns, requestId }, 200, request);
        }
      }
    ...
    if (op === 'putimg') {
      ...
      return json({ ok: true, ns: ns, k: k, size: data.length, hash: hash, imageRev: imageRev, updatedAt: updatedAt, requestId }, 200, request);
    }
...
async function saveIncomingAsFork(request, env, ctx, o) {
  const fkind = 'fork:' + String(o.pkg.device || 'dev').replace(/[^\w\-\.\(\)]/g, '').slice(0, 24) + ':' + Date.now();
  await env.DB.prepare('INSERT OR REPLACE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)')
    .bind(o.user, fkind, o.curRev, (o.baseRev == null ? 0 : o.baseRev), +o.pkg.updatedAt || Date.now(), String(o.pkg.device || ''), o.str.length, o.str).run();
  ctx.waitUntil(trimForks(env, o.user));
  return json({ ok: true, fork: true, rev: o.curRev, server: { rev: o.curRev, updatedAt: o.curUpdatedAt || 0, device: o.curDevice || '' }, ns: o.ns, requestId: o.requestId }, 200, request);
}

// ★v16: D1 .run() 結果から「行が変わったか」を判定(RETURNING行 or meta.changes)。
function d1Changed(r) {
  if (!r) return false;
  if (r.results && Array.isArray(r.results)) return r.results.length > 0;
...
export { verifyGoogleIdToken, b64urlToBytes, b64urlToStr, handleSave, d1PutImg, d1Changed, d1FirstRev, saveIncomingAsFork };
