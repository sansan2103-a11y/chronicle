...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 14, d1: !!env.DB, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true }, 200, request);
    }
    ...
    // ---------- ★v14: /save : クロスデバイス・セーブ同期(D1保存・KVフォールバック) ----------
    //   v10-13はKV保存(無料枠 書込1000/日・全キー合算 で長時間プレイ/複数人に耐えない)。
    //   v14はセーブ本体をD1(SQLite・書込10万行/日・強整合=約60秒stale解消)へ移行。
    //   ・op:meta/get/put は後方互換(fix399がそのまま動く)。D1未バインド時は従来のKV動作(デプロイ安全)。
    //   ・op:put は rev/baseRev の楽観ロック対応(fix402): 分岐(fork)は絶対に上書きせず両方保持。
    //   ・op:forceput = 現mainをforkに退避してから上書き(黙って消さない)。op:forks/getfork = 分岐の閲覧。
    //   ・画像はD1に入れず従来どおりKV split key(img:<ns>:<k>)+GET /img 配信(v11-13)のまま。
    //   ・KVのsave:<user>本体はもう書かない(旧クライアントの読取と/img遅延展開のため残置)。
    if (path === '/save') {
      if (!env.LEDGER) return json({ error: 'LEDGER(KV)が未バインドです' }, 503, request);
      ...
      if (op === 'meta') {
        ...
        return json({ ok: true, meta, rev: meta ? (+meta.rev || 0) : 0, ns, v: 14, d1: !!d1 }, 200, request);
      }
      ...
      if (op === 'get' || op === 'getfork') {
        if (d1) {
          ...
          if (row && row.blob) { let data = null; try { data = JSON.parse(row.blob); } catch (e) {} return json({ ok: true, data, rev: +row.rev || 0 }, 200, request); }
          if (op === 'getfork') return json({ ok: false, error: 'fork not found' }, 404, request);
        }
        const raw = await env.LEDGER.get(skey);
        return json({ ok: true, data: raw ? JSON.parse(raw) : null, rev: 0 }, 200, request);
      }

      if (op === 'put' || op === 'forceput') {
        const pkg = body.pkg;
        if (!pkg || typeof pkg !== 'object') return json({ error: 'no pkg' }, 400, request);
        const ns = await ensureNs(env, user);
        // 画像はKV split key(配信用)へ。D1行には入れない(行を軽く保つ)。
        const idb = (pkg.idb && typeof pkg.idb === 'object' && Object.keys(pkg.idb).length) ? pkg.idb : null;
        if (idb) { ctx.waitUntil((async () => { for (const kk of Object.keys(idb)) { try { await env.LEDGER.put('img:' + ns + ':' + kk, String(idb[kk])); } catch (e) {} } })()); }
        if (d1) {
          const light = {}; for (const k in pkg) { if (k !== 'idb') light[k] = pkg[k]; }
          const str = JSON.stringify(light);
          if (str.length > 4 * 1024 * 1024) return json({ error: 'セーブが大きすぎます(4MB超)' }, 413, request);
          ...
          if (op === 'put' && hasBase && cur && baseRev < curRev) {
            // ★分岐(fork): 相手を消さない。incoming側をforkとして保存し、サーバー現行(main)を維持。
            const fkind = 'fork:' + String(pkg.device || 'dev').replace(/[^\w\-\.\(\)]/g, '').slice(0, 24) + ':' + Date.now();
            await env.DB.prepare('INSERT OR REPLACE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)')
              .bind(user, fkind, curRev, baseRev, +pkg.updatedAt || Date.now(), String(pkg.device || ''), str.length, str).run();
            ctx.waitUntil(trimForks(env, user));
            return json({ ok: true, fork: true, rev: curRev, server: { rev: curRev, updatedAt: cur ? (+cur.updatedAt || 0) : 0, device: cur ? String(cur.device || '') : '' }, ns }, 200, request);
          }
          if (op === 'forceput' && cur && cur.blob) {
            // 現行mainをforkに退避してから上書き(黙って消さない)
            const bkind = 'fork:' + String(cur.device || 'dev').replace(/[^\w\-\.\(\)]/g, '').slice(0, 24) + ':' + (+cur.updatedAt || Date.now());
            await env.DB.prepare('INSERT OR REPLACE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)')
              .bind(user, bkind, curRev, +cur.baseRev || 0, +cur.updatedAt || 0, String(cur.device || ''), +cur.size || 0, cur.blob).run();
            ctx.waitUntil(trimForks(env, user));
          }
          const newRev = curRev + 1;
          await env.DB.prepare('INSERT OR REPLACE INTO saves (u, kind, rev, baseRev, updatedAt, device, size, blob) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)')
            .bind(user, 'main', newRev, hasBase ? baseRev : curRev, +pkg.updatedAt || Date.now(), String(pkg.device || ''), str.length, str).run();
          return json({ ok: true, rev: newRev, size: str.length, ns }, 200, request);
        }
        ...
        if (fullStr.length > 24 * 1024 * 1024) return json({ error: 'セーブが大きすぎます(24MB超)' }, 413, request);
        ...
        return json({ ok: true, size: fullStr.length, ns }, 200, request);
      }

      if (op === 'forks') {
        if (!d1) return json({ ok: true, forks: [] }, 200, request);
        ...
        return json({ ok: true, forks: (rs && rs.results) || [] }, 200, request);
      }

      // ★v13: 画像1枚だけ更新(split key直書き)。v14: 正準ユーザーのnsに書く(名寄せ対応)。
      if (op === 'putimg') {
        const k = String((body && body.k) || '').slice(0, 128);
        const data = String((body && body.data) || '');
        if (!k || !data) return json({ error: 'k,data required' }, 400, request);
        if (data.length > 2 * 1024 * 1024) return json({ error: 'image too large' }, 413, request);
        const ns = await ensureNs(env, user);
        await env.LEDGER.put('img:' + ns + ':' + k, data);
        return json({ ok: true, ns: ns, k: k, size: data.length }, 200, request);
      }
      return json({ error: 'unknown save op (get|put|forceput|meta|putimg|forks|getfork)' }, 400, request);
    }
...
async function handleImg(request, env, ctx) {
  ...
  let dataUrl = await env.LEDGER.get('img:' + ns + ':' + k);   // 個別キー(速い)
  if (!dataUrl) {                                              // 無ければ本体blobから遅延展開して全キー補填
    ...
    if (raw) {
      ...
      if (idb && typeof idb === 'object') {
        dataUrl = idb[k] || null;
        ctx.waitUntil((async () => { for (const kk of Object.keys(idb)) { try { await env.LEDGER.put('img:' + ns + ':' + kk, String(idb[kk])); } catch (e) {} } })());
      }
    }
  }
