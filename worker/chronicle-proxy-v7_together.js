// ============================================================
// Chronicle APIプロキシ (Cloudflare Worker) — v7
// v6からの追加(2026-07-02):
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      return json({ ok: true, service: 'chronicle-proxy', v: 7, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID }, 200, request);
    }
      ...
      if (tgKey) {
        try {
          ...
          if (tgResp.ok) {
            ...
            if (tb64) return json({ data: [ { b64_json: tb64 } ], provider: 'together' }, 200, request);
            ...
            if (!sub.ok) {
              ...
              if (!rid) {
                ...
                if (done) {
                  ...
                  if (b64out) return json({ data: [ { b64_json: b64out } ], provider: 'fireworks-kontext' }, 200, request);
