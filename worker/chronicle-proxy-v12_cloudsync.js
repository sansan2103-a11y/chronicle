...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 12, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true }, 200, request);
    }
    ...
    if (path === '/save') {
      ...
      return json({ error: 'unknown save op (get|put|meta)' }, 400, request);
    }
...
async function handleImg(request, env, ctx) {
  ...
  if ((request.headers.get('If-None-Match') || '') === etag) {
    return new Response(null, { status: 304, headers: imgHeaders(request, { 'ETag': etag, 'Cache-Control': 'public, max-age=300' }) });
  }
  ...
  return new Response(arr, { status: 200, headers: imgHeaders(request, {
    ...
    'Cache-Control': 'public, max-age=300',   // 5分保持+ETag再検証。regenは最大5分で反映(手動同期で即時)
    'ETag': etag,
  }) });
}
