...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      return json({ ok: true, service: 'chronicle-proxy', v: 10, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID }, 200, request);
    }
    ...
    if (path === '/save') {
      ...
      if (op === 'meta') {
        ...
        return json({ ok: true, meta: m ? JSON.parse(m) : null }, 200, request);
      }
      if (op === 'get') {
      ...
      if (op === 'put') {
        ...
        return json({ ok: true, size: str.length }, 200, request);
      }
