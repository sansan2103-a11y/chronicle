...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ ok: true, service: 'chronicle-proxy', v: 15, d1: !!env.DB, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID, img: true }, 200, request);
    }
    ...
    if (path === '/save') {
      ...
      if (op === 'meta') {
        ...
        return json({ ok: true, meta, rev: meta ? (+meta.rev || 0) : 0, ns, v: 15, d1: !!d1, requestId }, 200, request);
      }
      ...
      if (op === 'put' || op === 'forceput') {
        ...
        const ns = await ensureNs(env, user);
        // ★v15: 画像は D1 images(d1時)/KV split key(非d1) へ。saves 行には入れない(行を軽く保つ)。
        ...
        if (d1) {
          ...
          if (op === 'put' && hasBase && cur && baseRev < curRev) {
            ...
            return json({ ok: true, fork: true, rev: curRev, server: { rev: curRev, updatedAt: cur ? (+cur.updatedAt || 0) : 0, device: cur ? String(cur.device || '') : '' }, ns, requestId }, 200, request);
          }
      ...
      if (op === 'putimg') {
        ...
        if (d1) {
          const r = await d1PutImg(env, ns, k, data);   // ★v15: KVに書かず D1 images へ(書込予算根絶)
...
async function d1PutImg(env, ns, k, data) {
  ...
  const cur = await env.DB.prepare('SELECT rev FROM images WHERE ns=?1 AND k=?2').bind(ns, k).first();
  const rev = (cur ? (+cur.rev || 0) : 0) + 1;
  const hash = String(str.length) + ':' + smallHash(str);
  const now = Date.now();
  await env.DB.prepare('INSERT OR REPLACE INTO images (ns, k, rev, hash, updatedAt, data) VALUES (?1,?2,?3,?4,?5,?6)').bind(ns, k, rev, hash, now, str).run();
