...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      return json({ ok: true, service: 'chronicle-proxy', v: 8, ledger: !!env.LEDGER, google: !!env.GOOGLE_CLIENT_ID }, 200, request);
    }
    ...
    if (path === '/image') {
      ...
      if (tgKey) {
        try {
          ...
          if (tgResp.ok) {
            ...
            if (tb64) {
              ctx.waitUntil(recordImg(env, 'together')); // ★v8: 枚数記帳
            ...
            if (!sub.ok) {
              ...
              if (!rid) {
                ...
                if (done) {
                  ...
                  if (b64out) {
                    ctx.waitUntil(recordImg(env, 'fireworks')); // ★v8
      ...
      ctx.waitUntil(recordImg(env, 'pollinations')); // ★v8
