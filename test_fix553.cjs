...
console.log('\n== 指標(metrics) ==');
{
  ...
  /* ★fix558: この作品は文の区切りに U+2015 「―」 も使う(全体で62回)。
     数えていなかったので、すでに区切られている文を「句読点なし118字」と誤検出していた。
     実測: 崩れターン 8→6 / 崩れ区間 18→14。 */
  ok('★★「―」(U+2015)を区切りとして数える',
     M('砂塵を含んだ風がいつの間にか変わっていた\u2015肌寒かった夜風ではなくなるのに変わり果てたらしい熱を持っていたのに違いなかったようだった\u2015それは真横を通っていくとひやりとした体温差を与えたように思われただろう').over80 === 0,
     M('砂塵を含んだ風がいつの間にか変わっていた\u2015肌寒かった夜風ではなくなるのに変わり果てたらしい熱を持っていたのに違いなかったようだった\u2015それは真横を通っていくとひやりとした体温差を与えたように思われただろう'));
  ok('★「；」も区切りとして数える',
     M('あ'.repeat(60) + '；' + 'い'.repeat(60)).over80 === 0);
  ok('★「―」を足しても本物の崩れは見逃さない', M('あ'.repeat(262)).over80 === 1);
}
